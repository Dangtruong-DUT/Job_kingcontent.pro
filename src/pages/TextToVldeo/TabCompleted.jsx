...
import { fetchCompletedVideos, parseVideoToPost } from "@/utils/utilityFunc";
...
const TabCompleted = ({ parent, handleAddToWaitingList = null }) => {
    ...
    const {
        ...
        completedTotalPage,
        completedTotalCount,
    } = useSelector((state) => state.textToVideo);
    ...
    const handleScheduleVideo = (video) => {
        dispatch(setSelectedScheduleContent(parseVideoToPost(video)));
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                ...
                {!isLoadingCompleted &&
                    videosCompleted.map((item, index) => {
                        const isSelected =
                            autoWaitingList.contents.filter(
                                (x) => x.source_type == "video_ai" && x.videos[0] == item.video_url
                            ).length > 0;
                        return (
                            <div
                                key={index}
                                className={`bg-white shadow-md p-2 rounded-md relative group h-72 flex flex-col ${
                                    isSelected ? "border-green-700 border-4 bg-green-500" : "bg-gray-100"
                                }`}
                            >
                                ...
                                <div className="absolute right-3 top-3 flex gap-1 items-center">
                                    {parent === ParentType.SourceIdeas && (
                                        <button
                                            className={`px-1 py-1 rounded-lg hover:bg-white`}
                                            onClick={() => handleScheduleVideo(item)}
                                        >
                                    ...
                                    {parent === ParentType.SourceIdeasAuto && (
                                        <button
                                            className={`px-2 py-1 text-base text-white rounded-md hover:bg-gray-500 ${
                                    ...
                                    {parent === ParentType.TextToVideo && (
                                        <button
                                            className=""
                                            onClick={() => handleDownloadVideo(item.video_url, `${item.file_id}.mp4`)}
                                        >
