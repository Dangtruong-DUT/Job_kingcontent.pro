...
import {
    ...
    setCaptionBackgroundCorner,
    setLogoFile,
    setAudioFile,
    setLogoPosition,
    setOuttroFile,
    setOuttroDuration,
    setIsLoadingGenerateScript,
...
const TabCreateVideo = ({
    handleFetchPendingCompletedVideos,
    dataVoices,
    dataFonts,
    dataLanguages,
    dataMusics,
    ...
    const handlePromptToScript = async (replaceScriptConfirmed) => {
        ...
        if (script && !replaceScriptConfirmed) {
            confirmAlert({
                ...
                message:
                    "Lá»i thoáº¡i má»›i sáº½ Ä‘Æ°á»£c tá»± Ä‘á»™ng thay vÃ o Ã´ nháº­p lá»i thoáº¡i bÃªn dÆ°á»›i, báº¡n cÃ³ muá»‘n tiáº¿p tá»¥c khÃ´ng ?",
                buttons: [
                    {
            ...
            try {
                var res = await TextToVideoService.PromptToScript(
                    prompt,
                    scriptStyle,
                    scriptLength,
                    scriptLanguageCode
    ...
    const handleUploadOuttroFile = async (e) => {
        ...
        if (file.type.startsWith("video")) {
            ...
            try {
                ...
                tempVideo.src = url;
                tempVideo.load();
                while (tempVideo.readyState < 1) await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    useEffect(() => {
        setPreviewClassName(
            `${"text-" + ["xs", "sm", "base", "lg", "xl"][fontSize - 1]} ${fontBold ? "font-bold " : ""}`
        );
        setPreviewStyle({
            color: "#" + fontColor,
            fontFamily: fontName,
    ...
    useEffect(() => {
        ...
        return () => {
            if (voiceAudioRef?.current) voiceAudioRef.current.removeEventListener("ended", handleEnded);
        };
    }, [voiceAudioUrl]);
        ...
        <div>
            ...
            <div className="rounded-lg p-2 mb-4" style={{ background: "#FF9B9B" }}>
                <span className="text-xl font-bold">
                    Vui lÃ²ng háº¡n cháº¿ táº¡o ká»‹ch báº£n vá» má»™t nhÃ¢n váº­t cá»¥ thá»ƒ, sáº£n pháº©m cá»¥ thá»ƒ
                    hay bá»‘i cáº£nh cá»¥ thá»ƒ cÃ³ tháº­t nÃ o Ä‘Ã³, vÃ¬ AI cÃ³ thá»ƒ lÃ m sai lá»‡ch hÃ¬nh áº£nh vá»›i
                    thá»±c táº¿!
                </span>
            </div>
            ...
            <div className="mt-6 grid grid-cols-12 gap-3">
                ...
                <div className="flex flex-col col-span-2 justify-end">
                    ...
                    <select
                        className="h-10 mt-2 border rounded-md p-2 text-small"
                ...
                <div className="flex flex-col col-span-2 justify-end">
                    ...
                    <select
                        className="h-10 mt-2 border rounded-md p-2 text-small"
                        onChange={(e) => dispatch(setScriptLength(Math.round(Number(e.target.value))))}
                        value={scriptLength}
                    >
                ...
                <div className="flex flex-col col-span-2 justify-end">
                    ...
                    <select
                        className="h-10 mt-2 border rounded-md p-2 text-small"
                        onChange={(e) => dispatch(setScriptStyle(e.target.value))}
                        value={scriptStyle}
                    >
                ...
                <div className="flex flex-col col-span-2 justify-end">
                    ...
                    {isLoadingGenerateScript ? (
                        ...
                        <button
                            className="h-10 text-center text-lg font-bold py-2 px-4 rounded-md cursor-pointer mx-2 bg-blue-500 text-white hover:bg-blue-600 hover:shadow-lg transition duration-100"
                            onClick={() => handlePromptToScript()}
                        >
                            Táº¡o lá»i thoáº¡i
                        </button>
                    )}
                    ...
                    <div className="flex justify-between items-center">
                        <div className="w-2/4">
                            <span className="text-2xl font-semibold mb-4">
                                BÆ°á»›c 2 : Táº¡o video tá»« lá»i thoáº¡i
                            </span>
                        </div>
                    </div>
                    {/* DÃ²ng 2 */}
                    <div className="mt-3">
                        <div className="mt-3 flex gap-4 items-center h-12">
                            ...
                            <div className="w-1/4 text-left">
                                <span
                                    className={`text-base font-semibold mb-4 ${titleWordCount > 130 && "text-red-500"}`}
                                >
                                    {titleWordCount > 130 &&
                                        "Há»‡ thá»‘ng chá»‰ nháº­n tiÃªu Ä‘á» tá»‘i Ä‘a 130 tá»«. Äá»™ dÃ i hiá»‡n táº¡i : "}{" "}
                                    {titleWordCount}/130
                                </span>
                            </div>
                            <div className="w-1/4 text-right">
                                <span
                                    className={`text-base font-semibold mb-4 ${
                                        scriptWordCount > 1000 && "text-red-500"
                                    }`}
                                >
                                    {scriptWordCount > 1000 &&
                                        "Há»‡ thá»‘ng chá»‰ nháº­n ká»‹ch báº£n tá»‘i Ä‘a 1,000 tá»«. Äá»™ dÃ i hiá»‡n táº¡i : "}{" "}
                                    {scriptWordCount}/1000
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-center border border-gray-500">
                            <div
                                className={`border border-gray-500 flex ${
                                    horizontalAlignment == HORIZONTAL_ALIGNMENT.LEFT.value
                                        ? "justify-start"
                                        : horizontalAlignment == HORIZONTAL_ALIGNMENT.CENTER.value
                                        ? "justify-center"
                                        : horizontalAlignment == HORIZONTAL_ALIGNMENT.RIGHT.value
                                        ? "justify-end"
                                        : ""
                                }
                ${
                    verticalAlignment == VERTICAL_ALIGNMENT.TOP.value
                        ? "items-start"
                        : verticalAlignment == VERTICAL_ALIGNMENT.CENTER.value
                        ? "items-center"
                        ...
                        : ""
                            ...
                            >
                                <div className="p-0.5" style={previewBackgroundStyle}>
                ...
                <div className="mt-1 grid grid-cols-5 gap-5 flex flex-col">
                    ...
                    <div className="flex flex-col gap-6 col-span-2">
                            ...
                            <div className="w-2/3">
                                ...
                                <div className="flex gap-2 items-center w-full">
                                    <Select
                                        ...
                                        value={
                                            dataVoices.find((v) => v.name === voiceName)
                                                ? {
                                                      value: voiceName,
                                                      label: voiceName,
                                        ...
                                        formatOptionLabel={(option) => (
                                            <div className="flex items-center hover:bg-gray-300 rounded p-2">
                                                <span className="font-semibold">
                                                    {option.data.language_code === "vi-VN"
                                                        ? "Tiáº¿ng Viá»‡t"
                                                        : `(${option.data.language})`}
                                                </span>
                                                <span className="">
                                                    : {option.data.gender === "MALE" ? "Nam" : "Ná»¯"}
                                                </span>
                                                <span className="font-normal">- {option.data.name}</span>
                                                {option.data.is_new === true && (
                                                    <img
                                                        src={new_icon}
                                                        alt="icon"
                                                        className="inline-block ml-1 w-8 h-4"
                                                    />
                                                )}
                                            </div>
                                        )}
                        ...
                        <div className="flex gap-4">
                            <div className="flex items-end w-64">
                                <div className="w-full border rounded-md p-2 flex items-center">
                                    <input
                                        type="checkbox"
                                        className="form-checkbox w-6 h-6 rounded-md mr-3"
                                        onChange={(e) => dispatch(setIsCaptionVisible(Boolean(e.target.checked)))}
                                        checked={isCaptionVisible}
                                    />
                            ...
                            <div className="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <select
                                            className="w-12 border rounded-md p-2 text-small pl-1"
                                            onChange={(e) => dispatch(setStrokeWeight(Number(e.target.value)))}
                                            value={strokeWeight}
                                        >
                                        ...
                                        <input
                                            type="color"
                                            className="w-10 h-10 border rounded-md pl-1"
                                            onChange={(e) => dispatch(setStrokeColor(e.target.value.replace("#", "")))}
                                            value={"#" + strokeColor}
                                        />
                                    </div>
                                </div>
                        ...
                        {isCaptionVisible === true && (
                            <div>
                                ...
                                <div className="flex gap-4 items-center">
                                    <select
                                        className="w-32 border rounded-md p-2 text-small"
                                        onChange={(e) => dispatch(setFontName(e.target.value))}
                                        value={fontName}
                                    >
                                        {dataFonts.map((item) => {
                                            return (
                                    ...
                                    <select
                                        className="w-24 border rounded-md p-2 text-small"
                                    ...
                                    <div className="w-32 border rounded-md p-2 flex items-center">
                                        <input
                                            type="checkbox"
                                            className="form-checkbox mr-3 w-6 h-6 rounded-md"
                                            onChange={(e) => dispatch(setFontBold(Boolean(e.target.checked)))}
                                            checked={fontBold}
                                        />
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => dispatch(setFontColor(e.target.value.replace("#", "")))}
                                        value={"#" + fontColor}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="col-span-3">
                                ...
                                <div className="flex gap-4 items-center">
                                    ...
                                    <input
                                        ...
                                        className="w-10 h-10 border rounded-md"
                                        onChange={(e) =>
                                            dispatch(setCaptionBackgroundColor(e.target.value.replace("#", "")))
                                        }
                                        value={"#" + captionBackgroundColor}
                                    />
                                    <span className="text-gray-700 text-base mb-1">Äá»™ Ä‘áº­m</span>
                                    <select
                                        className="w-20 border rounded-md p-2 text-small pl-1"
                                        onChange={(e) => dispatch(setCaptionBackgroundOpacity(Number(e.target.value)))}
                                    ...
                                    <select
                                        className="w-20 border rounded-md p-2 text-small pl-1"
                                        onChange={(e) => dispatch(setCaptionBackgroundCorner(Number(e.target.value)))}
                                        value={captionBackgroundCorner}
                                    >
                                        {Array.from({ length: 5 }, (_, i) => i + 1).map((value) => {
                                            return (
                                                <option key={value} value={value}>
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="col-span-3">
                                ...
                                <div
                                    ...
                                    onClick={() => {
                                        if (logoFile != null) dispatch(setLogoFile(null));
                                        else document.getElementById("upload-logo").click();
                                    }}
                                >
                    ...
                    <div className="flex flex-col col-span-2">
                        ...
                        {audioUrlPlay && (
                            <div className="mt-3 flex items-center max-w-lg">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        dispatch(setMusicTitle(null));
                                        dispatch(setMusicCate(null));
                        ...
                        <div className="mt-3 flex items-center max-w-lg">
                            ...
                            <select
                                className="w-4/12 border rounded-md text-small ml-3 mr-3"
                                onChange={(e) => dispatch(setMusicCate(e.target.value))}
                                value={musicCate}
                            >
                        ...
                        <div className="mt-3 w-full max-w-lg bg-white shadow-lg rounded-lg p-4 max-h-96 overflow-y-auto">
                            {(!musicCate ? dataMusics : dataMusics.filter((music) => music.cate == musicCate)).map(
                                (music) => (
                                    <div className="p-2 w-full flex items-center">
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                dispatch(setMusicTitle(music.title));
                                                dispatch(setMusicCate(music.cate));
                                                dispatch(setAudioFile(null));
                                            }}
                                        >
                                            Chá»n
                        ...
                        <div className="mt-6 w-full max-w-lg flex flex-col">
                            <span className="text-base font-semibold">
                                Upload clip quáº£ng cÃ¡o cá»§a báº¡n cuá»‘i video (tá»‘i Ä‘a 50MB)
                            </span>
                            {outtroFile && (
                                <div className="mt-3 flex items-center">
                                    <span className="w-3/4 text-base text-green-700 font-semibold pr-1">
                                        {outtroFile.name} - {outtroDuration?.toFixed(1)}s -{" "}
                                        {(outtroFile.size / (1024 * 1024)).toFixed(2)} MB
                                    </span>
                                    <button
                                        className="w-1/4 px-3 py-2 bg-red-500 text-white rounded text-base font-semibold hover:bg-red-600 transition duration-200"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            dispatch(setOuttroFile(null));
                                        }}
                                    >
                                        XoÃ¡
                                    </button>
                            ...
                            >
                                {outtroFile ? (
                                    ...
                                    <span className="text-gray-500 text-sm w-full text-center p-3">
                                        Nháº¥n Ä‘á»ƒ chá»n file video hoáº·c áº£nh
                                    </span>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col col-span-1">
                        {generateMode == MODE.STANDARDSTOCK.value ? (
                            <span className="text-base font-bold text-red-500 italic">
                                Phong cÃ¡ch video chá»‰ kháº£ dá»¥ng vá»›i Äá»‹nh dáº¡ng video: áº¢nh hoáº¡t hÃ¬nh AI
                                <br />
                        ...
                        <div
                            ...
                            className={`mt-3 w-full bg-white shadow-lg rounded-lg overflow-y-auto relative ${
                                generateMode === MODE.STANDARDSTOCK.value
                        ...
                        >
                            {Object.values(STYLES).map((style) => (
                                <div className="p-2">
                                    <div
                                        className={`relative w-full flex items-center rounded-lg cursor-pointer transition-all duration-200 ${
                                            videoStyle === style.prompt
                                                ? "border-4 border-double border-green-500"
                                                : "hover:bg-gray-200"
                                        }`}
                                        onClick={() =>
                                            generateMode !== MODE.STANDARDSTOCK.value &&
                                            dispatch(setVideoStyle(videoStyle === style.prompt ? null : style.prompt))
                                        }
                                    >
                                        {/* áº¢nh phong cÃ¡ch */}
                                        ...
                                        <div
                                            className={`absolute bottom-1 left-1 ${
                                                videoStyle == style.prompt ? "bg-green-500" : "bg-black bg-opacity-50"
                        ...
                        <div className="w-full h-full flex flex-col items-center py-1 px-1 rounded-md mx-2 bg-blue-500 text-white">
                            {progress != 100 ? (
                                ...
                                <div className="flex items-center">
                                    <span className="text-lg font-bold">Äang xá»­ lÃ½...</span>
