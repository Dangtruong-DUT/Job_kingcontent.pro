            ...
            if (response.status === OK) {
                ...
                setImportFonts(
                    `https://fonts.googleapis.com/css2?${fonts
                        .map((item) => `family=${item}:wght@400;700`)
                        .join("&")}&display=swap`
                );
            }
        } catch (error) {
            ...
            <div className="min-h-screen flex justify-center bg-gray-100 p-4">
                <div className="max-w-3xl bg-white rounded-lg shadow-xl p-8 text-center">
                    ...
                    <p className="text-lg text-gray-700 leading-relaxed">
                        Sever tạo video AI tại nước ngoài đang gặp sự cố, không trả được tín hiệu. Chúng tôi đang gấp
                        rút liên hệ để tìm hiểu lí do và khắc phục. Rất mong nhận được sự thông cảm và kiên nhẫn từ các
                        sếp! Chúng tôi sẽ thông báo ngay khi đối tác nước ngoài phản hồi. Xin trân trọng cảm ơn!
                    </p>
        ...
        <div className="min-h-screen bg-transparent text-gray-900">
            ...
            <div className="p-4 flex justify-center sticky top-16 z-50 shadow-md rounded-md bg-white">
                <ul className="flex w-full gap-3">
                    <li
                        className={`flex-1 text-center shadow-md text-lg font-bold py-2 px-4 rounded-md cursor-pointer ${
                            activeTab === TABS.GENERATE ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-700"
                        }`}
                        onClick={() => dispatch(setActiveTab(TABS.GENERATE))}
                    >
                        🎥 Tạo Video
                    </li>
                    <li
                        className={`flex-1 text-center shadow-md py-2 px-4 rounded-md cursor-pointer ${
                            activeTab === TABS.PENDING ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-700"
                        }`}
                        onClick={() => dispatch(setActiveTab(TABS.PENDING))}
                    >
                        <span className="text-lg font-bold">⏳ Đang Chờ</span>
                        <span className="text-base font-base ml-1">({textToVideoState.videosPending?.length})</span>
                    </li>
                    <li
                        className={`flex-1 text-center shadow-md py-2 px-4 rounded-md cursor-pointer ${
                            activeTab === TABS.COMPLETED ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-700"
                        }`}
                        onClick={() => dispatch(setActiveTab(TABS.COMPLETED))}
                    >
                        <span className="text-lg font-bold">✅ Hoàn Tất</span>
                        <span className="text-base font-base ml-1">({textToVideoState.completedTotalCount})</span>
                    </li>
                </ul>
            </div>
            ...
            <div
                className={`my-2 ${
                    textToVideoState.activeTab === TABS.GENERATE ? "bg-white rounded-m p-4" : "bg-transparent"
                }`}
            >
