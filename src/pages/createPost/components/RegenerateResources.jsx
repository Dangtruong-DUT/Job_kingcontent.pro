...
const RegenerateResources = () => {
    ...
    const onRegenerate = async () => {
        ...
        if (selectedSource == SPECIAL) {
            generateContent(
                specialContentsState.contents
                    .filter((item, index) => specialContentsState.selectedIndexs.includes(index))
                    .map((item, index) => {
                        return {
    ...
    const generateContent = async (contents) => {
        ...
        var res = await CreateContent.generateContents(
            createPostState.planSelect.id,
            createPostState[KEY_LABEL_SELECT],
            contents
        );
        if (res.status === OK) {
            ...
            CreateContent.generateAContent(
            ...
            {selectedSource == null ? (
                ...
                <>
                    ...
                    {selectedSource == SPECIAL ? (
                        <Special
                            isMultiSelect={true}
                            allowedContentTypes={["for_ai_generate", "image", "video", "text"]}
                            additionalButton={renderButtonGenerate()}
                        />
