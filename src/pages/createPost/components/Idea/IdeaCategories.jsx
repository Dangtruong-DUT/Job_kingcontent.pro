                ...
                if (cateType === "takeCare") {
                    ...
                    filteredCats = availableData.reduce((acc, cat) => {
                        if (parseInt(cat?.parent_id) === DEFAULT_TAKE_CARE_CAT) {
                            ...
                            const childCats = availableData.filter((item) => item.parent_id === cat.cate_id);
                            if (childCats?.length) {
                                acc.push(...childCats);
                            }
                        }
                ...
                filteredCats = filteredCats.reduce((acc, cat) => {
                    ...
                    if (cate_name.toLowerCase().includes(keyword)) {
                        if (parent) {
                            ...
                            const parentCat = [...availableData].find((item) => item.cate_id === parent.cate_id);
    ...
    return (
        <>
            {isLoading ? (
                ...
                <>
                    <p className="bold italic mb-2">* Chọn chủ đề bạn muốn tham khảo ý tưởng</p>
