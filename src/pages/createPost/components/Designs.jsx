...
import { KEY_EDITOR_IMAGE, KEY_HASH_VIDEO_OR_IMAGE, KEY_INDEX_IMAGE_SELECT } from "@/reducers/createContent";
...
const Designs = (props) => {
            ...
            <div className="designsContainer">
                <div className="grid grid-cols-3 mt-3 gap-2">
                    {designs && designs.length > 0 ? (
                            ...
                            return (
                                ...
                                >
                                    <div className="absolute top-0 h-0 flex-col w-full overflow-hidden justify-center group-hover:h-full group-hover:flex">
                                        ...
                                        >
                                            {imageSelect.some((elt) => elt.id === _elt.id) ? "Đã chọn" : "Chọn"}
                                        </button>
                                    </div>
                                </div>
                            );
                        ...
                        <h4 className="font-bold text-center">Không tìm thấy mẫu ảnh gợi ý</h4>
                    )}
                </div>
                ...
                <div className="flex justify-center mt-4">
                    {designCurrentPage < designTotalPages && (
                        <button
                            ...
                            onClick={() => dispatch(actionGetDesigns(designCurrentPage + 1, catId))}
                        >
                            Tải thêm
                        </button>
                    )}
    ...
    return (
        <div className="mb-2 p-2">
            ...
            <DialogImage open={open} setOpen={setToggleDialog} imageLink={imgSelect} />
        </div>
    );
};
