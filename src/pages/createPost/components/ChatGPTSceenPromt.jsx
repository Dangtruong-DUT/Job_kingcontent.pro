...
import { BANK, CHAT, DOWNLOAD, PEDDING, _TAB_CHAT_GPT_PROMT } from "@/pages/createPost/utility";
...
const ChatGPTSceenPromt = () => {
    const { syncRequestPedding, historyQuestionList } = useSelector((state) => state.createPost);
    ...
    const renderScreen = () => {
        switch (_tabSelected) {
            ...
            case BANK:
                return <BankQuestionPromt setInputValue={setInputValue} setTabSelected={setTabSelected} />;
            case PEDDING:
                return <SyncRequest />;
            case DOWNLOAD:
                return <Saved />;
                    ...
                    {_TAB_CHAT_GPT_PROMT.map(({ title, icon, type }, index) => (
                        <li
                            className={`"mr-2 p-4 border-b-2 rounded-t-lg flex items-center hover:border-blue-700 ${
                                _tabSelected === type && "border-blue-700 transition-all duration-700 cursor-pointer"
                            }`}
                            id="profile-tab"
                            role="presentation"
                            key={index}
                        ...
                        >
                            <div className="relative flex justify-center items-center">
                                {type === PEDDING && (
                                    <div className="absolute text-green">{syncRequestPedding.length}</div>
                                )}
                                {type === DOWNLOAD && (
                                ...
                                <img
                                    src={icon}
                                    width={35}
                                    className={type === PEDDING && syncRequestPedding.length > 0 ? "animate-spin" : ""}
                                />
                            </div>
                            <button
                                className={`${
