...
const SingleDes = (props) => {
    ...
    useEffect(() => {
        if (selectedDestinations && destination && selectedDestinations.length > 0) {
            const search = selectedDestinations.filter(
                (item) => item.id === destination?.id || (item.id === destination?.username && type === "tiktok")
            );
        ...
        if (destination) {
            ...
            switch (type) {
                case "threads":
                    ...
                    setAvatar(convertInstagramLink(threads_profile_picture_url) || threadsIcon);
                    break;
