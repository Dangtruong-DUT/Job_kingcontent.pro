...
const ListSchedules = (props) => {
    const { selectedSchedule, setSelectedSchedule, scheduleName, setScheduleName } = props;

    const [isShowBody, setIsShowBody] = useState(true);
    const [scheduleOptions, setScheduleOptions] = useState([]);

    const { isSchedulesLoading = false, schedules = null } = useSelector((state) => state.schedules);
    ...
    return (
        <div className={`schedulesContainer m-2 ${isShowBody ? "" : "overflow-hidden"}`}>
            <div
            ...
            <div className={`body   duration-300 ease-in-out ${isShowBody ? "h-auto p-3 border" : "h-0"}`}>
                <Select
