...
const Elements = ({
    ...
    const { [KEY_ELEMENTS_IMAGES]: elements } = useSelector((state) => state[REDUX_NAME_CREATE_POST]);
    ...
    const bringForward = (customId) => {
        ...
        if (elements.length > 1) {
            ...
            dispatch(UpdateCanvasLayer(_layers));
        }
        ...
        <div className="relative bg-gray-700 text-white overflow-hidden  w-96 py-2 px-2">
            <div className="flex justify-end">
                <IoMdCloseCircle
                    ...
                    onClick={() => setLayerStatus(false)}
                />
            </div>
            <div className="text-white flex items-center gap-3">
                <FaHistory size={25} color="#ffffff" />
                <span>Lịch sử</span>
            </div>

            <PerfectScrollbar className="flex flex-col mt-2" style={{ height: "35vh" }}>
                {history.length === 0 ? (
                    ...
                    history.map((_elt, index) => (
                        <p
                            key={index}
                            className={`px-2 hover:bg-gray-800 duration-200 rounded-md cursor-pointer flex items-center justify-between ${
                                activeHistoryIndex === index && "bg-gray-800"
                            }`}
                        >
            ...
            <div className="text-white flex items-center gap-3 mt-2">
                <FaLayerGroup size={25} color="#ffffff" />
                <h2>Thành phần</h2>
            </div>
            {isArrayEmpty(elements) ? (
                <div className="flex items-center flex-col">
                ...
                <PerfectScrollbar className="flex flex-col mt-2" style={{ height: "35vh" }}>
                    {elements.map((_elt, index) => (
                        <p
                            key={index}
                            className={`px-2 hover:bg-gray-800 duration-200 rounded-md cursor-pointer flex items-center justify-between ${
                                activeObjects === _elt.customId && "bg-gray-800"
                            }`}
                        >
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="flex">
                                            {index !== 0 && (
                                                <FaLongArrowAltUp
                                                    size={20}
                                                    onClick={() => bringForward(_elt.customId)}
                                                    className="hover:bg-red-600 duration-200 ease-linear rounded-md p-1"
                                                />
                                            )}
                                        </div>
                                        <div
                                            className="flex items-center gap-2"
                                            onClick={() => onSelectObject(_elt.customId)}
                                        >
                                  ...
                                  _elt.type === "triangle" ? (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div
                                            ...
                                            onClick={() => onSelectObject(_elt.customId)}
                                        >
                                            {_elt.iType === "circle" || _elt.type === "circle" ? (
                                                <RiCheckboxBlankCircleFill size={20} color={_elt.fill} />
                                            ) : _elt.iType === "rectangle" ||
                                              (_elt.width > _elt.height && !_elt.points && !_elt.lineCoords) ? (
                                                <MdRectangle size={20} color={_elt.fill} />
                                            ) : _elt.iType === "square" ||
                                              (_elt.width === _elt.height && !_elt.points && !_elt.lineCoords) ? (
                                                <FaSquareFull size={20} color={_elt.fill} />
                                            ) : _elt.iType === "arrow" || (_elt.points && _elt?.points.length === 9) ? (
                                                <FiArrowDownRight size={20} color={_elt.fill} />
                                            ) : _elt.type === "triangle" ? (
                                                <IoTriangleSharp size={20} color={_elt.fill} />
                                            ) : _elt.points && _elt?.points.length === 6 ? (
                                                <MdHexagon size={20} color={_elt.fill} />
                                            ) : (
                                                <FaGripLines size={20} color={_elt.fill} />
                                            )}
                                            <span>Khối</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        >
                                            {/* <IoImage size={20} /> */}
                                            <img
                                                src={_elt.imageData ? _elt.imageData : _elt.src}
                                                alt=""
                                                height={30}
                                                width={30}
                                            />
