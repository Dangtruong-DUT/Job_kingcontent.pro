...
const Draw = ({ editor }) => {
    ...
    return (
        <div>
            <div className="flex items-center gap-5">
                <div className="w-1/2">
                    ...
                    <div className="h-16 flex items-center">
                        <input
                            type="color"
                            className="w-full h-10 mt-3 rounded-md"
                            value={color}
            ...
            <PerfectScrollbar className="flex flex-wrap mt-3" style={{ maxHeight: "70vh" }}>
                {lists.map((_elt, index) => (
                    <div
                        className={`w-1/2 flex flex-col mb-5 items-center h-20 justify-center cursor-pointer hover:bg-gray-800 rounded-md duration-200 ease-out py-2 ${
                            selected === index && "bg-gray-800"
                        }`}
                        onClick={() => onSelectOption(index, _elt.width, _elt.color)}
                        key={index}
                    >
                        <div>{_elt.icon}</div>
