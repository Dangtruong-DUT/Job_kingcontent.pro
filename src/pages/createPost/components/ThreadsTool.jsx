...
const ThreadsChatGpt = (props) => {
    const { isCopied, setIsPopupTag, setIsContent, togglePopup, handleCopyToClipBoard } = props;
    const [hasPending, setHasPending] = useState(false);

    const dispatch = useDispatch();
    const { textContent, threadsChatGptData } = useSelector((state) => state.createPost);

    useEffect(() => {
        const hasPending = threadsChatGptData.some((item) => item.status === "PENDING");
    ...
    const onClickOptimize = () => {
        confirmAlert({
            title: "XÃ¡c nháº­n",
            message: "Báº¡n cÃ³ cháº¯c cháº¯n muá»‘n tá»‘i Æ°u ná»™i dung?",
            buttons: [
                {
                    label: "CÃ³",
                    onClick: () => {
    ...
    const handleDeleteQuestion = (id) => {
        confirmAlert({
            title: "XÃ¡c nháº­n",
            message: "Báº¡n cÃ³ cháº¯c cháº¯n muá»‘n xoÃ¡ ná»™i dung nÃ y?",
            buttons: [
                {
                    label: "XoÃ¡",
                    onClick: () => {
                        const _newData = threadsChatGptData.filter((_elt) => _elt.id !== id);
    ...
    const renderSingleItem = (id, title, convertTitle, answer, isSaved = false) => {
        return (
            <div className="bg-gray-100 rounded-lg shadow-sm p-3 mb-4">
                <div>
                    <h3 className="font-bold text-base line-clamp-1">{convertTitle}</h3>
                    <div className="line-clamp-3">
                        <span>{answer}</span>
                    </div>
                </div>
                <div className="flex justify-end gap-5 mt-3">
                    <button onClick={() => handleReplaceQuestion(title, id)} title="Äá»•i káº¿t quáº£">
                        <FiRefreshCcw size="25" className="text-black" />
                    </button>
                    <button onClick={() => togglePopup(title, answer, id)} title="Xem thÃªm">
                        <FiMaximize size="25" color="#FF8B13" className="hover:text-green-500" />
                    </button>
                    {isCopied.status && isCopied.id === id ? (
                        <FiCheckCircle size="25" color="green" className="hover:text-green-500" />
                    ) : (
                        <button
                            title="ÄÆ°a vÃ o trÃ¬nh soáº¡n tháº£o"
                            onClick={() => handleCopyToClipBoard(answer, id)}
                        >
                            <FiEdit size="25" color="green" className="hover:text-green-500" />
                        </button>
                    )}
                    ...
                    <button onClick={() => handleDeleteQuestion(id)} title="XoÃ¡ káº¿t quáº£">
                        <FiTrash size="25" color="red" className="hover:text-green-500" />
                    </button>
                </div>
            </div>
        );
    ...
    return (
        <div>
            <p className="italic text-center font-bold">
                * Tá»‘i Æ°u ná»™i dung á»Ÿ trÃ¬nh soáº¡n tháº£o dÃ nh riÃªng cho ná»n táº£ng Threads!
            </p>

            <div className="flex justify-center mt-2">
                {textContent.length > 0 ? (
                    <p>
                        Sá»‘ lÆ°á»£ng kÃ½ tá»±: <span className="font-bold">{textContent.length}</span> /{" "}
                        <span className="text-red-500 font-bold">500</span>
                    </p>
                ) : (
                    <p className="text-red-500">Vui lÃ²ng soáº¡n ná»™i dung trÆ°á»›c khi tá»‘i Æ°u</p>
                )}
            </div>
                ...
                {threadsChatGptData.length > 0 && (
                    <div className="grid grid-cols-1 gap-2">
                        {threadsChatGptData.map((item, index) => {
                            ...
                            const convertedTitle = title.replace(`${defaultThreadsPrompt} `, "Ná»™i dung gá»‘c: ");
                            if (status === "PENDING") {
                                return (
                                    <div
                                    ...
                                    >
                                        <div>
                                            <h3 className="font-bold text-base line-clamp-1">{convertedTitle}</h3>
...
const DownloadTool = (props) => {
    ...
    useEffect(() => {
        const _threadsHistory = historyQuestionList.filter((item) => item.type === "threads");
        ...
        if (currentKeyword || currentTag) {
            ...
            if (currentKeyword) {
                _filteredList = _filteredList.filter(
                    (item) => item.question.includes(currentKeyword) || item.answer.includes(currentKeyword)
                );
            }
                ...
                {filteredList.length > 0 ? (
                    <div className="grid grid-cols-1 gap-2">
                        {filteredList.map((item, index) => {
                            ...
                            return (
                                <div className="bg-gray-100 rounded-lg shadow-sm p-3 mb-4" key={index}>
                                    <div>
                                        <h3 className="font-bold text-base line-clamp-1">{convertedQuestion}</h3>
                                        <div className="line-clamp-3">
                                    ...
                                    <div className="flex justify-between gap-5 mt-3 items-center">
                                        {tag ? (
                                            <span
                                                ...
                                                title="Chá»‰nh sá»­a"
                                                onClick={() => onClickUpdateTag(question, answer, id, tag)}
                                            >
                                                {tag}
                                            </span>
                                        ) : null}
                                        <div className="gap-5 flex ml-auto items-center">
                                            ...
                                            <button onClick={() => togglePopup(question, answer, id)} title="Xem thÃªm">
                                                <FiMaximize
                                                    size="25"
                                                    color="#FF8B13"
                                                    className="hover:text-green-500"
                                            ...
                                            {isCopied.status && isCopied.id === id ? (
                                                ...
                                                <button onClick={() => handleCopyToClipBoard(answer, id)}>
                                                    <FiEdit
                                                        size="25"
                                                        color="green"
                                                        className="hover:text-green-500"
                    ...
                    <div className="flex justify-center items-center h-96">
                        <p>Báº¡n hÃ£y bá» lá»c hoáº·c thÃªm dá»¯ liá»‡u má»›i</p>
                    </div>
                )}
            </PerfectScrollbar>
        </div>
...
const ThreadsTool = () => {
    ...
    const handleSaveHistory = (editingHashtag = null) => {
        if (editingHashtag) {
            dispatch(updateHistoryHashtag(editingHashtag.oldTag, editingHashtag.newTag));
            setIsPopupTag(false);
        } else {
            const obj = {
    ...
    return (
        <div className="p-4">
                    ...
                    return (
                        <div
                            ...
                            className={`mr-2 p-4 border-b-2 rounded-t-lg flex items-center hover:border-blue-700 relative  cursor-pointer ${
                                tabSelected === tab.type && "border-blue-700 transition-all duration-700"
                            }`}
                        >
                            ...
                            {tab.type === DOWNLOAD && (
                                <div className="absolute top-2 left-8 w-3 h-3 text-xs">
                                    {historyQuestionList.length > 0 &&
                                        historyQuestionList.filter((item) => item.type === "threads").length}
                                </div>
                            )}
            ...
            <PopupDetailContentChat
                ...
                question={isContent?.question?.replace(`${defaultThreadsPrompt} `, "Ná»™i dung gá»‘c: ")}
                isCopied={isCopied}
