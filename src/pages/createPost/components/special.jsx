...
const Special = ({ isMultiSelect = false, allowedContentTypes = null, additionalButton = null }) => {
    ...
    if (allowedContentTypes) defaultContentType = allowedContentTypes[0];
    ...
    const onSearch = () => {
        // prepare query
        let query = "";
        if (contentType) query += `&media_type=${contentType}`;
        else query += `&media_type=${defaultContentType}`;
    ...
    return (
        <div className="p-2 specialContents">
            ...
            <form className="filtering p-3" onSubmit={onSearch}>
                <div className="filter whitespace-nowrap flex items-center gap-2 mb-3" htmlFor="sort">
                    <input
                        type="text"
                        id="search"
                ...
                <div className="mt-1 mb-3 grid grid-cols-3 gap-3">
                    ...
                    <KindOfContentSelect
                        kindOfContentToSearch={setContentType}
                        validOptionValues={allowedContentTypes}
                    />
                </div>
                <div className="mb-3">
                    <label className=" text-sm font-semibold text-gray-800 filter drop-shadow-md" htmlFor="sort">
                        Sắp xếp dữ liệu:
                    </label>
