...
const Upload = ({ setIsEditor, images, setImages }) => {
    ...
    const upload = async () => {
        if (hashVideoOrImage === UPLOAD_TYPE_VIDEO && !isArrayEmpty(imageSelect)) {
            toast.warning("Không thể sử dụng nhiều video trong cùng một content, vui lòng xóa video cũ");
            return;
        }
        setUploading(true);
        ...
        const resCreateResources = await ResourcesService.uploadFile(formData, setUploadProgress);
        if (resCreateResources.status === OK) {
            const sliceArray = resCreateResources?.data?.data?.slice(0, newData.length);
    ...
    const handleChangeTab = (type) => {
        if ((type === 1 && !isArrayEmpty(imageSelect)) || (type === 1 && !isArrayEmpty(images))) {
            confirmAlert({
                title: "Thông báo",
                message: "Không thể sử dụng cả ảnh và video trong cùng một content, vui lòng xóa hết ảnh !",
                buttons: [
                    {
                        label: "Xoá hết ảnh",
                        onClick: async () => {
        ...
        if ((type === 0 && !isArrayEmpty(imageSelect)) || (type === 0 && !isArrayEmpty(videos))) {
            confirmAlert({
                title: "Thông báo",
                message: "Không thể sử dụng cả ảnh và video trong cùng một content, vui lòng xóa hết video !",
                buttons: [
                    {
                        label: "Xoá hết video",
                        onClick: async () => {
    ...
    const handleOnSelectResource = (resource) => {
        if (resource.type === "image") {
            if (hasVideo(imageSelect)) {
                toast.warning("Không thể sử dụng video và ảnh trong cùng một content, vui lòng xóa video cũ");
                return;
            }
    ...
    const handleOnRemoveResource = async (resource) => {
        ...
        confirmAlert({
            title: "Thông báo",
            message: `Bạn có chắc muốn xoá ${type === "image" ? "ảnh" : "video"} này không ?`,
            buttons: [
                {
                    label: "Có",
                    onClick: async () => {
                            ...
                            >
                                <div className="flex flex-col justify-center items-center pt-5 pb-6">
                                    ...
                                    <p className="mb-2 text-gray-500 dark:text-gray-400">
                                        <span className="font-semibold">Nhấn hoặc kéo thả hình ảnh để tải lên</span>
                                    </p>
            ...
            <>
                <div className="grid grid-cols-4 gap-4 mb-3">
                    {videos.map((video, index) => (
                        <div key={index} className="image-item relative">
                            <video src={video.url} controls className="h-full w-full rounded-lg max-h-52" />
                        ...
                        {resources &&
                            ...
                            resources.map((resource, index) => {
                                ...
                                return (
                                    <div key={index} className="image-item relative">
                                        {type === "image" ? (
                                            <img src={url} alt="" className="h-full w-full rounded-lg max-h-52" />
                                        ) : (
                                            <video src={url} controls className="h-full w-full rounded-lg max-h-52" />
                                        )}
    ...
    useEffect(() => {
        if (editItem) {
            ...
            setSelectedIndex(
                editItem?.media_type === UPLOAD_TYPE_IMAGE ? 0 : editItem?.media_type === UPLOAD_TYPE_VIDEO ? 1 : 0
            );
        }
    }, [editItem]);
    return (
        <div className="p-3 overflow-y-scroll max-h-full">
            <Tab.Group selectedIndex={selectedIndex} onChange={(e) => handleChangeTab(e)}>
                    ...
                    <Tab
                        ...
                        className={({ selected }) =>
                            classNames(
                                ...
                                selected ? "bg-white shadow" : "text-blue-100 hover:bg-white/[0.12] hover:text-black"
                            )
                        }
                    >
                        Hình ảnh
