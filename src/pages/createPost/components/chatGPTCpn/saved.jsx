...
const Saved = (props) => {
    ...
    const sortByTime = () => {
        setIsSort(!isSort);
        if (isSort) {
            const sortedData2 = originalHistoryList.sort((a, b) => moment(b.created).diff(moment(a.created)));
            dispatch({
                type: ACTION_GET_HISTORY_QUESTION,
                payload: sortedData2,
            });
        } else {
            const sortedData = originalHistoryList.sort((a, b) => moment(a.created).diff(moment(b.created)));
            dispatch({
                type: ACTION_GET_HISTORY_QUESTION,
                payload: sortedData,
            });
    ...
    useEffect(() => {
        if (historyQuestionList.length > 0) {
            const list = historyQuestionList.filter((item) => item.type !== "threads");
            setHistoryList(list);
        ...
        <>
            <div className="flex gap-3">
                ...
                <button
                    ...
                    onClick={sortByTime}
                >
                    {isSort ? <FaSortAmountDown size={23} /> : <FaSortAmountUp size={25} />}
                </button>
            </div>
            <section className="mt-4">
                ...
                {historyList.length > 0 ? (
                    <PerfectScrollbar className=" w-full mt-1" style={{ height: "450px" }}>
                                ...
                                <div className="flex justify-between gap-5 mt-3 items-center">
                                    ...
                                    <div className="gap-5 flex ml-auto items-center">
                                        ...
                                        {isCoppied.status && isCoppied.id === id ? (
                                            ...
                                            <button onClick={() => handleCoppyToClipBoard(answer, id)}>
                                                <FiEdit
                                                    size="25"
                                                    color="green"
