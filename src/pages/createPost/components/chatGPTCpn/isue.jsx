...
const Isue = () => {
    ...
    const { contentTitle, titleObjet, syncRequestPedding } = useSelector((state) => state.createPost);
    ...
    const handleGetContentChild = (keyword) => {
        ...
        toast.success("Đang xử lý câu hỏi , Vui lòng theo dõi tiến trình tại màn hình đang xử lý !");
            ...
            <div className="overflow-scroll overflow-x-hidden max-h-screen mb-20">
                {contentTitle.length === 0 && !isLoading ? (
                    <div className="flex justify-center">
                        <span className="font-bold mt-2  mb-72">Không có dữ liệu hiển thị</span>
                    </div>
                                    ...
                                    <div className="divide-y">
                                        {contentTitle.map((_elt, index) => (
                                            <div key={index} className="cursor-pointer flex flex-col">
                                                <div className="px-6 py-4 bg-gray-200  rounded-md">{_elt.title}</div>
