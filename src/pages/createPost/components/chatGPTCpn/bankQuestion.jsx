...
const BankQuestion = ({ setInputValue, setTabSelected }) => {
    ...
    const { bankQuestionList, syncRequestPedding } = useSelector((state) => state.createPost);
            ...
            {titleChildStatus ? (
                ...
                <div className="overflow-scroll overflow-x-hidden max-h-screen mb-20">
                    {bankQuestionList.map(({ name }, index) => (
                        ...
                        >
                            <span className={`pb-2 pt-2  pl-1 rounded-lg cursor-pointer font-bold`}>- {name}</span>
                            <span className="pr-2" onClick={() => handleGetContentChild(name)}>
