...
const PopupCreatePlan = ({
    ...
    const { [KEY_HASH_TAG_PLANS]: tags, [KEY_IMAGES]: images } = useSelector((state) => state.createPost);
    ...
    const onAddUserTag = () => {
        if (userTags.length >= maxUserTag) {
            toast.error("Số lượng cột content tối đa là 10, vui lòng tạo kế hoạch mới !");
            return;
        }
        const newUserTag = {
            ...userTag,
    ...
    const onSubmit = async (data) => {
        ...
        if (!isEdit) {
            if ((data.productName === null || data.productName === "") && isAdvance && tagsSelect.size !== 0) {
            ...
            const res = await CreateContent.updatePlan(editItem.id, getBodyCreatePlan(data));
            if (res.status === OK) {
    ...
    const getAllImage = async () => {
        ...
        if (res.status === OK) {
            const imageFilter = res.data.data.filter((_elt) => _elt.type === UPLOAD_TYPE_IMAGE);
            dispatch(
                updateProps([
                    {
                        prop: KEY_IMAGES,
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 mb-3 mt-2">
                ...
                <InputSwitch checked={isAdvance} onChange={(e) => setIsAdvance(e.value)} className="switchCustom" />
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex gap-5">
                    <div className={isAdvance ? "w-1/2" : "w-full"}>
                        ...
                        <p className={`${_text_title} text-blue-500`}>Chọn màu nền hoặc hình nền</p>
                        <Tab.Group>
                            ...
                            <Tab.Panels className="mt-2 overflow-x-hidden max-h-screen">
                                <Tab.Panel>
                                    <div
                                        className={`mb-2 ${
                                            watchAllFields.thumbnail !== null ? "pointer-events-none opacity-50" : ""
                                        } `}
                                    >
                                ...
                                <Tab.Panel className="mt-2 overflow-scroll overflow-x-hidden max-h-screen">
                                    <div
                                        className={`mb-2 ${
                                            watchAllFields.color !== null ? "pointer-events-none opacity-50" : ""
                                        } `}
                                    >
                                        ...
                                        >
                                            <img src={uploadCloud} alt="kingcontent upload file" className="h-24" />
                                        </label>
                                        <PerfectScrollbar
                                            ...
                                            {images.map((_elt, index) => (
                                                <div
                                                    ...
                                                    className={`p-1 h-28 relative rounded-lg border-2 shadow-lg cursor-pointer ${
                                                        watchAllFields.thumbnail === _elt.url ? "border-red-500" : ""
                                                    }`}
                                                    onClick={() => {
                    ...
                    {isAdvance && (
                        <div className="w-1/2">
                            <div className="mb-2">
                                ...
                                <div className="mt-2">
                                    <span className={`${_text_title} text-blue-500`}>CÁC CỘT CONTENT BÊN TRONG</span>
                                    <PopupSuggTagChatgpt
                                ...
                                {tagsSelect.size || userTags.length ? (
                                    <>
                                        <p className={`${_text_title} text-blue-500 mt-2`}>Tên sản phẩm</p>
                ...
                <div className="flex justify-center mt-5 gap-2">
                    <button
                        ...
                        type="submit"
                    >
                        {isLoadingCreatePlan ? "Đang xử lý ..." : isEdit ? "Cập nhật" : "Tạo mới"}
                    </button>
                </div>
            </form>
        </Dialog>
