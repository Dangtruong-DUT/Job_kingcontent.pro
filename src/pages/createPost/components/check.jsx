...
import { actionToggleReplaceContent, toggleIsCheckVPCS } from "@/store/actions/createContent";
import PopupSelectKeyReplace from "./popupSelectKeyReplace";
const Check = () => {
    const { keyworksCheck, contentAvailabel, keyWordsHasInContent, textContent } = useSelector((state) => {
    ...
    const replaceKeyVPCS = () => {
        const confirmRepl = confirm("Báº¡n cháº¯c cháº¯n muá»‘n thay tháº¿ toÃ n bá»™ ?");
        if (confirmRepl) {
            ...
            for (let i = 0; i < keyworksCheck.length; i++) {
                keyworksCheck.forEach((element) => {
                    ...
                    const obj = keyRepl.reduce((a, v) => ({ ...a, [original]: keyRepl[random] }), {});
    ...
    const replaceKeySelectVPCS = (_key) => {
        const confirmRepl = confirm("Báº¡n cháº¯c cháº¯n muá»‘n thay tháº¿ tá»« nÃ y ?");
        if (confirmRepl) {
    ...
    const toggleOpenPopupRepl = (key) => {
        ...
        if (key === "") {
            ...
            setKeySelect(key);
            const list = keyworksCheck.find((_elt) => _elt.original === key).replacement.split(",");
            setListKeyRepl(list);
        }
    };
    useEffect(() => {
            ...
            {keyWordsHasInContent.length === 0 ? (
                <div className="flex justify-center">
                    <span className="font-bold">
                        Má»i chuyá»‡n cÃ³ váº» á»•n, chÃºng tÃ´i hiá»‡n chÆ°a tÃ¬m tháº¥y tá»« khÃ³a nÃ o Ä‘Ã¡ng ngá»
                        VPCS cá»§a Facebook
                    </span>
                </div>
            ) : (
                <div className="p-3 list-disc relative" style={{ minHeight: "200px" }}>
                    <span>
                        Nhá»¯ng tá»« nÃ y cÃ³ thá»ƒ vi pháº¡m chÃ­nh sÃ¡ch cá»§a facebook khi cháº¡y quáº£ng cÃ¡o :
                    </span>
