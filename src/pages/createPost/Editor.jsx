...
import { changeStateFeedbacksForm, setCurrentFeedback } from "@/store/actions/editor/editorActions";
...
`;
const Editor = ({ collapse, setCollapse, setTitleType, setIsEditor, setIsRightImage, setImagesRightEditor }) => {
    ...
    useEffect(() => {
        if (isReels) {
            dispatch(updateProps([{ prop: KEY_HASH_VIDEO_OR_IMAGE, value: "video" }]));
        }
    }, [isReels]);
    ...
    const { isShowFeedbacksForm = false, savedFeedbacks = [] } = useSelector((state) => state.editor);
    ...
    const handleCreatePost = async () => {
        ...
        const isReels = imageSelect.some((_elt) => _elt.can_be_reels === 1);
    ...
    return (
        <div>
            {isSaving && (
                <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-9999 flex justify-center items-center">
                    <div className="p-5 bg-white rounded-lg flex gap-3 items-center">
                        ...
                        <p className="text-black text-xl">Đang lưu bài viết, vui lòng chờ trong giây lát !</p>
                    </div>
                </div>
            )}
            <EditorStyled className="gap-2">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="pr-3 flex gap-3">
                                <button onClick={() => dispatch(openScreenPlan())}>
                                    <FaThList size={25} color="green" title="Danh sách kế hoạch" />
                                </button>
                                {collapse ? (
                                    <button title="Hiện công cụ hỗ trợ" onClick={() => setCollapse(false)}>
                                        <TbLayoutSidebarLeftCollapse size={30} color="green" />
                                    </button>
                                ) : (
                                    <button title="Ẩn công cụ hỗ trợ" onClick={() => setCollapse(true)}>
              ...
              >
                ...
                {textContent.length > 0 ? (
                    <div className="text-right py-3 text-base font-bold">( {textContent.length} kí tự )</div>
                ) : null}
                ...
                {isActivePreset && textContent.length > 130 ? (
                    <div className="bg-red-500 text-white p-2 text-center my-2">
                        <p>Bạn đang vượt quá {textContent.length}/130 kí tự, ảnh nền trên Facebook có thể sẽ bị mờ</p>
                    </div>
                ) : null}
                                ...
                                <div className="relative group" key={index}>
                                    ...
                                    (_elt.type && _elt.type === "video") ? (
                                        <VideoStyle className="rounded-md cursor-pointer">
                                            <div onClick={() => setVideoPopup(_elt.url ? _elt.url : _elt)}>
                                                <ReactPlayer
                                            ...
                                            <div
                                                ...
                                                onClick={() => setVideoPopup(_elt.url ? _elt.url : _elt)}
                                            >
                ...
                <div className="flex ml-2 gap-3 mt-3 bg-white justify-center items-center relative h-15 mb-5 p-2 rounded-lg shadow-sm">
                    ...
                    <input
                        ...
                        value={note}
                        onChange={(e) => dispatch(updateProps([{ prop: KEY_NOTE, value: e.target.value }]))}
                    />
                </div>
                <div className="ml-2 gap-3 flex mb-3">
                    <div
                        ...
                        style={{
                            backgroundColor: labelId ? labels.find((_elt) => _elt.id === labelId)?.color : "#fff",
                            color: labelId ? "#fff" : "#000",
                        }}
                    >
                        <FiTag size={25} />
                        <span className="font-bold">
                            {labelId === null ? " Phân loại content" : labels.find((_elt) => _elt.id === labelId)?.name}
                        </span>
                    </div>
                </div>
            ...
            {isShowSchedule && <ScheduleSettings setIsShowSchedule={setIsShowSchedule} />}
            {videoPopup && (
                <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-90 z-9999 flex justify-center items-center">
                    <button className="absolute right-2 top-2" onClick={() => setVideoPopup(null)}>
            ...
            <DialogImage open={openViewImage} setOpen={() => setOpenViewImage(!openViewImage)} imageLink={imgSelect} />
        </div>
    );
};
