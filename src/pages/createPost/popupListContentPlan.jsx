...
import { FiInfo, FiPlayCircle, FiPlusCircle, FiRefreshCcw, FiSave, FiSearch, FiX } from "react-icons/fi";
...
import { UPLOAD_TYPE_IMAGE, UPLOAD_TYPE_VIDEO, _input_style, getObject_create_post } from "./utility";
...
const PopupListContentPlan = ({ isOpen = true }) => {
    ...
    const contentDetailToShow = useSelector((state) => state.contents.contentDetailToShow);
    const contentCompare = useSelector((state) => state.contents.contentCompare);
        ...
        if (contentInPlan) {
            ...
            if (labels.length > 0) {
                const hasAutoSuggestion = labels.some(
                    (label) => label.contents.length === 1 && label.contents[0].post_text === "ADMIN_SUGGESTION"
                );
    ...
    const onDrop = async (evt, labelId) => {
        ...
        const dragItemContent = copyListItems[dragItem.current.parentIdex]["contents"][dragItem.current.subIndex];
        copyListItems[dragItem.current.parentIdex]["contents"].splice(dragItem.current.subIndex, 1);
        copyListItems[dragOverItem.current.parentIdex]["contents"].unshift(dragItemContent);
        ...
        const resUpdate = await CreateContent.updateLabelIdInContent(itemSelect, labelId);
    ...
    const checkHasOtherAIContents = (contentId) => {
        ...
        const AIContents = contentInPlan.labels.reduce((acc, label) => {
            const contents = label.contents.filter((content) => content?.saved === true);
            return [...acc, ...contents];
        }, []);
        // check if has other AI contents
        const hasAIContents = AIContents.some((content) => content.id !== contentId);
        return hasAIContents;
    };

    const handleEditContent = (content, labelId, saved) => {
        const { id, post_text, note, medias, media_type, media_url = "", wishlist = 0, is_reels = 0 } = content;
            ...
            confirmAlert({
                ...
                message: (
                    <span className="warning-content">
                        Vui lòng nhấn vào biểu tượng SAVE để lưu lại kết quả gợi ý trước khi thoát khỏi màn hình này để
                        tạo content mới
                    </span>
                ),
                buttons: [
    ...
    const handleBack = () => {
        if (hasAIContent) {
            confirmAlert({
                ...
                message: (
                    <span>
                        <span className="font-bold text-red-500">Vui lòng nhấn nút LƯU</span> bên dưới từng content để
                        giữ lại kết quả gợi ý!
                    </span>
                ),
                buttons: [
                    {
    ...
    const handleSavePost = async (post, labelId, planId, id) => {
        confirmAlert({
            title: "Cảnh báo !",
            message: <span className="warning-content">Bạn có chắc chắn muốn lưu nội dùng này !</span>,
            buttons: [
                {
                    label: "Xác nhận",
                    onClick: async () => {
                        const res = await CreateContent.createPost(
                            getObject_create_post(undefined, post.post_text, [], "", user._id, labelId, planId, "")
                        );
                        if (res.status === OK) {
                            const newData = { ...contentInPlan };
                            const labelsFind = newData.labels.findIndex((_elt) => _elt.id === labelId);
                            const index = newData.labels[labelsFind].contents.findIndex((item) => item.id === id);
    ...
    const handleDeleteContent = async (parentIdex, id, saved) => {
        confirmAlert({
            title: "Cảnh báo !",
            message: <span className="warning-content">Bạn có chắc chắn muốn xoá nội dùng này !</span>,
            buttons: [
                {
                    label: "Xác nhận",
                    onClick: async () => {
                        if (saved) {
                            const copyListItems = [...contentInPlan.labels];
                            const newArray = copyListItems[parentIdex].contents.filter((elt) => elt.id !== id);
        ...
        if (value !== "" && value) {
            ...
            const filterItem = copyListItems.map((_elt) => {
                const newArr = _elt.contents.filter((_sub) => {
                    if (_sub?.post_text.toLowerCase().includes(value) || _elt?.name.toLowerCase().includes(value)) {
                        return _sub;
                    }
                });
                return { ..._elt, contents: newArr };
    ...
    const handleCompareContent = async (content) => {
        dispatch(setContentCompare(content));
        ...
        return (
            <section className="lists-container">
                {labels.map((label, parentIndex) => {
                    const { name, color, contents, id: labelId, is_pin_to_top, keyword, note, is_generating } = label;
                    return (
                        <div
                            key={parentIndex}
                  ...
                  ${color}`,
                            ...
                            >
                                ...
                                <h4
                                    className={`font-bold uppercase text-center text-base select-none	cursor-not-allowed p-2 rounded-md ${
                            ...
                            <div className="flex justify-center mt-2">
                                {contents.length === 1 && contents[0].post_text === "ADMIN_SUGGESTION" && (
                                    <div className="flex justify-center mt-2" style={{ height: "280px" }}>
                                        <div className="flex justify-center items-center">
                                            {syncRequestPedding.some((_elt) => _elt.label_id === labelId) ? (
                                                <div>
                                                    ...
                                                    <div>
                                                        <p className="mt-2 font-bold px-5 text-center">
                                                            Vui lòng chờ chút để máy tính gợi ý kết quả cho bạn
                                                        </p>
                                                    </div>
                                                </div>
                                            ) : (
                                                <button
                                                    ...
                                                    onClick={() =>
                                                        handleQuestionChatGPT(`${keyword} ${contents[0].note}`, labelId)
                                                    }
                                                >
                                        ...
                                        (
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`border-2 mb-2 p-4 rounded-md shadow-sm cursor-move bg-white ${
                                                        post_text === "ADMIN_SUGGESTION" ? "hidden" : ""
                                                    }`}
                                                    onDragStart={(e) => onDragStart(e, parentIndex, index, id)}
                                                ...
                                                >
                                                    ...
                                                    <div className="flex justify-end gap-2 mt-2">
                                                        ...
                                                        >
                                                            <img src={editIcon} alt="" width={30} height={30} />
                                                        </button>
                                                        <button
                                                            className="Xoá nội dung"
                                                            onClick={() => handleDeleteContent(parentIndex, id, saved)}
                                                        >
                                                            <img src={trashIcon} alt="" width={30} height={30} />
                                                        </button>
                                                    </div>
                                                </div>
                                            );
    ...
    return (
        <Fragment>
            <Transition appear show={isOpen} as={Fragment}>
                <Dialog as="div" className="relative z-9999 w-screen mt-1" onClose={() => {}}>
                    <Transition.Child
                        as={Fragment}
                                ...
                                <Dialog.Panel className="w-full transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    <div className="w-full h-full bg-gray-100 p-2 mb-5 items-center">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="font-bold uppercase text-base">{plan?.name || ""}</span>
                                            <div className="flex gap-5">
                                                {plan && !/^\s*$/.test(textContent) && !isCreateToHomepage.status && (
                                                    <span
                                        ...
                                        <div className="flex justify-between gap-2">
                                            ...
                                            <div className="flex items-center relative" style={{ width: "60%" }}>
                                                <input
                                                    type="text"
                                                ...
                                                />
                                                <FiSearch className="absolute right-2" size={25} color="#000" />
                                            </div>
                                            <Select
                                                data={filterList}
            ...
            <PopupDetailContentPlan isOpen={isOpenDetail} setIsOpenDetail={setIsOpenDetail} item={itemViewDetail} />
            {/* <PopupSelectTag
