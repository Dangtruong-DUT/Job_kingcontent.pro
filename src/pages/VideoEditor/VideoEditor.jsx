    ...
    return (
        <div className="min-h-screen text-gray-900">
            ...
            <div className="flex bg-white rounded-md mb-4 p-6 items-center gap-3">
                {auth.isHasVip3() && (
                    <button
                        className="h-10 flex items-center px-4 rounded-lg bg-red-600 text-white transform shadow-sm hover:shadow-md hover:scale-105 transition-all duration-350"
            ...
            {isCheckingPermission ? <LoadingApp /> : hasPermission ? <ListVideos /> : null}

            {contentDetailToShow && <ContentDetail />}

            {showModal && (
                <ModalHashTag modalHashTagOpen={showModal} onClose={() => dispatch(actionSetShowModal(false))} />
            )}
        </div>
    );
}
