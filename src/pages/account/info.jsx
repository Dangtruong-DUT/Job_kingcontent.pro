...
const Info = () => {
    ...
    const renderRow = (title, value, diver = true) => {
        return (
            <div className={`flex justify-between ${diver && "border-b-2"} py-3 px-2`}>
    ...
    const handleLogout = () => {
        confirmAlert({
            title: "Đăng xuất",
            // @ts-ignore
            message: <span className="warning-content">Bạn có chắc chắn muốn đăng xuất ?</span>,
            buttons: [
                {
                    label: "Xác nhận",
                    onClick: async () => {
                ...
                <div className="bg-white rounded-lg mb-5 p-5 gap-2">
                        ...
                        <div className="card">
                            <Accordion activeIndex={0}>
                                <AccordionTab header="Thông báo hệ thống ">
                                    <p dangerouslySetInnerHTML={{ __html: text }} className="mb-5"></p>
                                </AccordionTab>
                            </Accordion>
                        </div>
                    </div>
                                    ...
                                    <div className="flex justify-between items-center hover:underline hover:text-red-500">
                                        <div
                                            className={`flex items-center ${
                                                type === INFO ? "font-bold text-red-500 cursor-pointer" : ""
                                            }`}
                                            onClick={() => changeType(INFO)}
                                        >
                                            {" "}
                                            <FaUserShield size={30} color={type === INFO ? "#EF4040" : "#000"} />
                                            <span className="ml-2 text-md cursor-pointer">Thông tin cá nhân</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="mb-5">
                                    <div className="flex justify-between items-center hover:underline hover:text-red-500">
                                        <div
                                            className={`flex items-center ${
                                                type === SECURITY ? "font-bold text-red-500 cursor-pointer" : ""
                                            }`}
                                            onClick={() => changeType(SECURITY)}
                                        >
                                            {" "}
                                            <FaLock size={25} color={type === SECURITY ? "#EF4040" : "#000"} />
                                            <span className="ml-2 text-md cursor-pointer">Cài đặt bảo mật</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="mb-5">
                                    <div className="flex justify-between items-center hover:underline hover:text-red-500">
                                        <div
                                            className={`flex items-center ${
                                                type === SOCIAL ? "font-bold text-red-500 cursor-pointer" : ""
                                            }`}
                                            onClick={() => changeType(SOCIAL)}
                                        >
                                            <FaFacebook size={25} color={type === SOCIAL ? "#EF4040" : "#000"} />
                                            <span className="ml-2 text-md cursor-pointer">{SOCIAL}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="mb-5">
                                    <div className="flex justify-between items-center hover:underline hover:text-red-500">
                                        <div
                                            className={`flex items-center ${
                                                type === LOGOUT ? "font-bold text-red-500 cursor-pointer" : ""
                                            }`}
                                            onClick={() => handleLogout()}
                                        >
                                            <IoLogOut size={35} color={type === LOGOUT ? "#EF4040" : "#000"} />
                                            <span className="ml-2 text-md cursor-pointer">{LOGOUT}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="w-3/4">
                            {type === INFO ? (
                                <>
                                    ...
                                    <div className="mt-2">
                                        {renderRow("Tên đầy đủ", `${user.first_name} ${user.last_name}`)}
                    ...
                    user.phone_number ? user.phone_number : ''
                                        ...
                                        {renderRow(
                                            "Ngày hết hạn",
                                            user.date_expired
                                                ? moment.unix(user.date_expired).utc(true).format("DD-MM-YYYY")
                                                : "",
                                            true
                                        )}
                                        {/* current package */}
                                ...
                                <div className={`socials w-full grid gap-2 ${isDev ? "grid-cols-3" : "grid-cols-2"}`}>
                                    <div className="">
                                        <Facebook user={user} />
                                    </div>
                                    <div className="">
