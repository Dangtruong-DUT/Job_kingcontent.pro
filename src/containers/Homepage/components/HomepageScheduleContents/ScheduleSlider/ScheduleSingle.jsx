    ...
    useEffect(() => {
        if (medias && medias.length > 0) {
            ...
            if (
                ...
                (scheduleContent?.source_content?.preset || scheduleContent?.source_content?.random_preset)
            ) {
                setThumbnail(
                    scheduleContent?.source_content?.preset?.url || scheduleContent?.source_content?.random_preset?.url
                );
            } else {
    ...
    const handleShowContentDetail = async () => {
        ...
        switch (source_type) {
            ...
            case "douyin":
                ...
                const res = await userServices.getSingleScheduleContent(scheduleContent.id);
                if (res && res.data) {
                    const { data } = res.data;
                    const {
                        text = "",
                    ...
                    let newContent = {
                        ...
                        user_screenname: scheduleContent.destination_name || user_display_name,
                        feed_id: destination_id,
            ...
            {empty ? (
                <div className="singleContent relative bg-white rounded-md w-full leading-10 p-4 flex h-24 cursor-pointer gap-4">
                    ...
                    <div className="w-full">
                        <p className="font-bold text-gray-100 text-base ">Kingcontent.pro</p>
                        <div className="border-b-2 border-dashed border-gray-100 w-full"></div>
                        <TextStyled className="mb-2"></TextStyled>
                    </div>
                    <div className="absolute w-full ">
                        <div className="flex mt-5 justify-center">
                            <button title="Click để tạo lịch đăng bài mới" onClick={() => redirectPage()}>
                ...
                >
                    ...
                    <div className="w-full">
                        <p className="font-bold text-base">
                            {destination_name || destination_id} {type === "threads" ? "(Threads)" : ""}
                        </p>
                        <div className="border-b-2 border-dashed border-gray-100 w-full"></div>
