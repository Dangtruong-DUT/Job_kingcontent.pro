...
const ScheduleSlider = (props) => {
    ...
    const { loadingScheduleContents = false, schedules } = useSelector((state) => state.homepage);
    ...
    useEffect(() => {
        dispatch(getScheduleContentsHomepage(6, moment().utc(true).format("YYYY-MM-DD")));
    }, [dispatch]);
        ...
        <div className="specialContents">
            <div className="sliderContents w-full">
                {loadingScheduleContents ? (
                    <div className="w-full h-96 flex justify-center items-center">Đang lấy dữ liệu...</div>
                ) : (
