...
const SingleContent = (props) => {
    ...
    const handelWritePost = async () => {
        if (media_type === "video") {
            ...
            dispatch(
                actionPushContentToCreateContentScreen(post_text, customMedias, media_type, wishlist === 1 || false)
            );
            history.push("/tao-content");
        } else {
            dispatch(resetCreateContent());
            dispatch(actionUpdateStep1(true));
            dispatch(actionPushContentToCreateContentScreen(post_text, medias, media_type, wishlist === 1));
            history.push("/tao-content");
        }
    };
    ...
    const handleSetSchedule = (item) => {
        ...
        // add next 7 minutes
        dispatch(setCurrentDateTime(moment().utc(true).add(7, "minutes").format("YYYY-MM-DD HH:mm:ss")));
    ...
    return (
        ...
        >
            ...
            <Footer className={"mt-2 justify-between"} likes={likes} comments={comments} shares={shares} />
            <PopupDetailContentPlan isOpen={isOpen} item={content} setIsOpenDetail={setiIsOpen} showTool={true} />
            ...
            <div className="actions absolute invisible rounded top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 py-3 px-5 gap-2 flex transition-all ease-in-out text-white z-20 flex-wrap group-hover:visible">
                ...
                >
                    <FiEye size={30} color="#fff" className="hover:scale-125 cursor-pointer" title="Xem chi tiết" />
                ...
                >
                    <FiCalendar size={30} color="#fff" className="hover:scale-125 cursor-pointer" title="Lên lịch" />
                    <span>Lên lịch</span>
                </div>
                {/* Tạo Video AI */}
                <div
