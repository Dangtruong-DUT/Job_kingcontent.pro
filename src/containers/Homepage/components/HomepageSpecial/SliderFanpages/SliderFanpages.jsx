            ...
            <div className="sliderFanpages w-full pt-1 ml-1 mt-4">
                {specialFanpages.length === 0 ? (
                    <div className="grid grid-cols-3 gap-2">
                        {[...Array(Number(3))].map((_elt, index) => (
                            <FanpageEmpty key={index} />
                        ))}
                    </div>
                ) : (
                    <Carousel infinite={true} responsive={responsive} autoPlay={true} autoPlaySpeed={4500}>
                        {specialFanpages &&
                            specialFanpages.length > 0 &&
                            specialFanpages.map((fanpage, idx) => <SingleFanpage fanpage={fanpage} key={idx} />)}
                    </Carousel>
                )}
            </div>
        </div>
