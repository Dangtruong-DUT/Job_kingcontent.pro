    ...
    const RenderTitle = (date, isHasEvent) => {
        return (
            <>
                {!isHasEvent ? (
                    <div>
                        Ngày <span className="font-bold text-red-600 w-full">{date}</span> chưa có sự kiện nào đặc biệt,
                        bạn có muốn tiếp tục tạo lịch đăng bài vào ngày này không
                    </div>
                ) : (
                    <div>
                        Có một sự kiện nhận được nhiều người quan tâm vào ngày{" "}
                        <span className="font-bold text-red-600 w-full">{date}</span>, bạn có muốn đăng bài theo dòng sự
                        kiện này không?
                    </div>
                )}
            </>
                ...
                buttons: [
                    {
                        ...
                        onClick: async () => {
                            ...
                            if (moment(date).isSame(moment(), "date")) {
                                ...
                                selectedDate = moment().add(10, "minutes").format("YYYY-MM-DD HH:mm");
                            }
                            if (isHasEvent) {
