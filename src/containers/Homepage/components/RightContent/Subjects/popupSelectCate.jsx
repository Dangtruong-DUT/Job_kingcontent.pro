...
const PopupSelectCate = ({ isOpen, toggle }) => {
    ...
    const handleChangeSearchCate = (input) => {
        if (input === "") handleGetAll();
            ...
            const result = availabelData.filter((_elt) => _elt.cate_name.toLowerCase().includes(input));
            setListContent(result);
        }
    };

    const handleSelectCategory = async (item) => {
        const isChosen = chosenCategories.find((element) => element.cate_id === item.cate_id);
        if (isChosen) {
                        ...
                        {!loading && listContent.length !== 0 ? (
                            ...
                            >
                                {listContent.map((_elt, index) => {
                                    ...
                                    const isChosen = chosenCategories.find((item) => item.cate_id === cate_id);
                                    return (
                                        <div className={`bg-gray-100 rounded-md mt-2 p-3 `} key={index}>
                                            <div className="flex justify-center">
                                                <div
                                                    className="thumbnail w-full h-40 block bg-no-repeat bg-cover"
                                                    style={{
                                                        backgroundImage: `url(${image_url || defaultImage})`,
                                                    }}
                                                >
                                                    {" "}
                                                </div>
                                            ...
                                            <div className="flex justify-center my-2">
                                                ...
                                                >
                                                    <FiFacebook />
                                                    {isChosen ? <span>Đã theo dõi</span> : <span>Theo dõi</span>}
                                                </button>
                                            </div>
                                        </div>
                                    );
            ...
            <div className="flex flex-wrap overflow-y-scroll gap-1 overflow-x-hidden">
                ...
                {parentListArr.map((_elt, index) => (
                    <div
                        key={index}
                        className={`border-2  hover:bg-gray-200 hover:text-black cursor-pointer rounded-lg p-4 ${
                            topicSelect === _elt?.cate_id ? "bg-blue-500 text-white" : "border-blue-200"
                        }`}
                    ...
                    <div className="fixed inset-0 overflow-y-auto">
                        <div className="flex min-h-full items-center justify-center p-4 text-center">
                            ...
                            >
                                ...
                                >
                                    <div className="mt-2">
                                        <h1 className="font-bold uppercase text-base">Danh sách chủ đề</h1>
