...
export const createPostReducers = (state = initialState, action) => {
    ...
    switch (type) {
        ...
        case ACTION_ADD_IMAGE: {
            ...
            if (feedbackId) {
                const index = imageSelect.findIndex((item) => item.feedbackId === feedbackId);
                if (index !== -1) {
        ...
        case GET_FACEBOOK_PRESETS_SUCCESS: {
            const { data: facebookPresetsData = [], current_page: currentPage, last_page: totalPages } = payload;
            return {
                ...state,
                facebookPresets:
                    currentPage > 1 ? [...state.facebookPresets, ...facebookPresetsData] : facebookPresetsData,
                isLoadingFacebookPresets: false,
        ...
        case SET_SELECTED_FACEBOOK_PRESET: {
            const newSeleted = state.selectedFacebookPreset === payload ? null : payload;
