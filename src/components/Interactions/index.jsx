...
const Interactions = (props) => {
    const { contentDetailToShow, totalFanpages, totalContents } = useSelector((state) => state.contents);
    ...
    useEffect(() => {
        if (takeCareFBCategories.length > 0) {
            // get random category
            const randomIndex = Math.floor(Math.random() * takeCareFBCategories.length);
    ...
    const onCategoriesSelected = (selected) => {
        ...
        setSelectedCategory(takeCareFBCategories.find((cat) => cat.cate_id === selected.value));
    };
