...
function SpecialFollowPopup(props) {
    ...
    const onPageChanged = async (page) => {
        ...
        const res = await client.get(
            currentQuery ? `/special-contents${currentQuery}&page=${page}` : `/special-contents?page=${page}`
        );
        if (res.status === OK) {
            setListPosts(res.data.data.data);
            setLoading(false);
    ...
    const renderContents = () => {
        if (listPosts && listPosts.length > 0) {
            return (
                <>
                    <GridLayoutContent currentContents={listPosts} page="specialFollowPage" />
                    <Pagination
    ...
    return (
        <div className="min-w-screen h-screen fixed inset-0 z-9999 flex items-center justify-center">
            ...
            >
                ...
                />
                <h2 className="text-base font-bold">{`${fanpage.page_name || "page name here"} (${
                    fanpage.total || 0
                } bài viết)`}</h2>
                <SearchAndFilter handleSearch={handleSearch} fanpage_id={fanpage.fanpage_id} getAllPost={getAllPost} />
                <PerfectScrollbar className="mt-7 text-nowrap -z-1">{renderContents()}</PerfectScrollbar>
            </div>
