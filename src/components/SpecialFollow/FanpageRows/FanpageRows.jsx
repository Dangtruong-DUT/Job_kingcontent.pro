...
import PopoverTag from "@/components/SpecialFollow/popoverTag";
...
const FanpageRows = (props) => {
    const { fanpages = [], setIsShowPopup, getFanpages, setFanpages, labels, handleChageHashTag } = props;
    ...
    const updateContent = async (fanpage) => {
        ...
        if (!is_queue) {
            ...
            if (res.status === OK) {
                toast.dismiss();
                toast.success("Vui lòng đợi trong ít phút để hệ thống tiến hành cập nhật");
    ...
    const deleteContent = async (fanpage) => {
        confirmAlert({
            title: "Cảnh báo !",
            message: <span className="warning-content">Bạn có chắc chắn muốn xoá trang này không?</span>,
            buttons: [
                {
                    label: "Xác nhận",
                    onClick: async () => {
