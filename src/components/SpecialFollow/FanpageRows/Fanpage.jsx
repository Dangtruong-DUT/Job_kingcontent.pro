...
const Fanpage = (props) => {
    const { fanpage, index, setIsShowPopup, setItemEdit, setVisible, visible, updateContent, deleteContent } = props;
    const {
        id = 0,
        fanpage_id = "",
        page_name = "",
    ...
    return (
        <div className={`w-full flex py-3 px-2 hover:bg-gray-100 ${index > 0 ? "border-t" : ""}`}>
            <div className="w-2/5 cursor-pointer" onClick={() => setIsShowPopup(fanpage)}>
            ...
            <div className="w-1/5 pt-7">
                ...
                <div className="w-1/3 float-left">
                    <div
                        className={`cursor-pointer ${is_queue ? "text-gray-500 cursor-not-allowed" : "text-green-500"}`}
                        onClick={() => updateContent(fanpage)}
                    >
                        <div className="float-right">Cập nhật</div>
                        <BiRefresh className="w-5 h-5 mr-2 float-right" fill={is_queue ? "grey" : "green"} />
                    </div>
                </div>
                <div className="w-1/3 float-left">
                    <div className="cursor-pointer" onClick={() => deleteContent(fanpage)}>
