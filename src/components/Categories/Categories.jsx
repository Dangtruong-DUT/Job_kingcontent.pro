...
const Categories = () => {
    ...
    const { childCategories, totalFanpages, totalContents } = useSelector((state) => state.categories);
                ...
                if (cateType === "takeCare") {
                    ...
                    filteredCats = childCategories.reduce((acc, cat) => {
                        if (parseInt(cat?.parent_id) === DEFAULT_TAKE_CARE_CAT) {
                            ...
                            const childCats = childCategories.filter((item) => item.parent_id === cat.cate_id);
                            if (childCats?.length) {
                                acc.push(...childCats);
                            }
                        }
                ...
                filteredCats = filteredCats.reduce((acc, cat) => {
                    ...
                    if (cate_name.toLowerCase().includes(filterKeyword)) {
                        ...
                        if (parent) {
                            ...
                            const parentCat = [...childCategories].find((item) => item.cate_id === parent.cate_id);
    ...
    useEffect(() => {
        if (chartLabels && chartLabels.length && chartDataSets && chartDataSets.length) {
            const newData = {
                labels: chartLabels,
                datasets: [
                    {
    ...
    return (
        <div className="categories">
            <CategoriesTitle totalContents={totalContents} totalFanpages={totalFanpages} />

            {chartData && <CategoriesChart chartData={chartData} />}
