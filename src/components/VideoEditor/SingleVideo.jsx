...
const SingleVideo = (props) => {
    ...
    const { id, video_url, process_status, rendered_url, created_at, description, hash_tag, thumbnail = "" } = video;
    ...
    return (
        <div className="bg-white shadow-md p-2 rounded-md relative group min-h-72 flex flex-col">
            <div className="w-full h-48 bg-gray-300 flex items-center justify-center rounded-md overflow-hidden">
                {thumbnail ? (
                    <img src={thumbnail} alt={`Thumbnail for video ${id}`} className="w-full h-full object-cover" />
                ) : (
                    <video src={video_url} className="w-full h-full object-cover" muted controls={false} />
                )}
                <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                    <ul className="actions flex flex-col gap-4">
                        {getActions(rendered_url).map(
                            (action) =>
                                action.condition && (
                                    <li key={action.id}>
                                        <button
            ...
            <div className="flex-1 gap-1 mt-2">
                ...
                <p className="text-xs italic text-gray-600">{new Date(created_at).toLocaleString()}</p>
            </div>
            ...
            <div className="absolute right-3 top-3 gap-1 flex items-center">
                {rendered_url && (
                    <button
                        ...
                        title="Táº£i video Ä‘Ã£ chá»‰nh sá»­a"
                    >
                        <img src={DownloadIcon} alt="Download" className="w-8 h-8" />
                    </button>
                )}
                <button className="" onClick={() => handleEditHashtag(video)} title="Chá»‰nh sá»­a hash tag">
                    <img src={HashtagIcon} alt="Hashtag" className="w-8 h-8" />
                </button>
                <button className="" onClick={() => handleDeleteVideo(id)} title="XÃ³a video">
