...
const SearchBox = (props) => {
    ...
    const { searchVideos = null, filterData, isFilter = false } = useSelector((state) => state.threads);

    const { scheduledContents = [] } = useSelector((state) => state.schedules);

    useEffect(() => {
        // check if no data on searchVideos || filterData
        if ((!searchVideos || searchVideos?.videos?.length === 0) && isFilter === false) {
            setHasNoData(true);
        } else {
            setHasNoData(false);
        }
    ...
    const onChangeFilter = (type, data) => {
        ...
        switch (type) {
            ...
            case "is_reels":
                newData = newData.filter((elt) => elt?.media_type === "video" && data.value);
                break;

            case "schedule":
                newData = newData.filter((elt) => {
                    const search = scheduledContents.find(
                        (item) => item?.content_id === elt?.code && item?.source_type === "threads"
                    );
    ...
    return (
        <div>
            <form className="flex justify-between gap-2 pt-3 mb-3" onSubmit={onSearch}>
                {/* trending button */}
                <button
          ...
          />
                ...
                {!isSchedule && (
                    <div>
                        <Label>LÆ°á»£t Ä‘Äƒng láº¡i</Label>
                        <Select
                            className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block"
                ...
                <div>
                    <Label>Video chuáº©n reels</Label>
                    <Select
                        className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block"
