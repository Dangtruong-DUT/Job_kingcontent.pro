...
const SinglePost = (props) => {
    const { post, handleAction, setIsShowComments, setCurrentPost } = props;
    const {
        code = "",
        comments = 0,
        likes = 0,
        ...
        <div className="flex flex-row border-b border-gray-200 cursor-pointer p-2 gap-3">
            <div className="w-28 flex justify-center items-center relative">
                {post.media_type === "image" ? (
                    <img src={images[0]} alt={text} className="w-24 h-24 object-cover rounded-md" />
                ) : post.media_type === "video" ? (
                    <Fragment>
                        <video src={videos[0]?.source} alt={text} className="w-24 h-24 object-cover" />
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                            <FaPlayCircle size={24} />
                        </div>
                    </Fragment>
                ) : (
                    <img src={getNoImage()} alt="" className="w-24 h-24 object-cover rounded-md" />
                )}
            </div>
            ...
            <div className="w-1/5 text-center flex justify-center items-center">{formatUnixDate(created)}</div>
            ...
            <div className="w-1/5 text-center flex gap-4 items-center justify-center">
                ...
                <a href="#" className="flex items-center gap-2" onClick={() => handleAction("CREATE_CONTENT", post)}>
                    <FaPencilAlt size={20} className="text-gray-500" />
                </a>
                <a href="#" className="flex items-center gap-2" onClick={() => handleAction("SCHEDULE_CONTENT", post)}>
                    <FaCalendarAlt size={20} className="text-gray-500" />
                </a>
                {/* new action to show all comments */}
                <a href="#" className="flex items-center gap-2" onClick={onClickShowComments}>
