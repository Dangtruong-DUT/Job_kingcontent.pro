...
const RightPart = (props) => {
        ...
        switch (action) {
                        ...
                        buttons: [
                            ...
                            },
                            {
                                ...
                                onClick: async () => {
                                    ...
                                    dispatch(actionPushContentToCreateContentScreen(text, medias, "video", is_reels));
                                    history.push("/tao-content");
                                },
                            },
                            {
            ...
            case "SCHEDULE_CONTENT": {
                if (elt.media_type === "video") {
                    ...
                    if (height >= minHeight && width >= minWidth && ratio >= minRatio && duration <= 90) {
                        dispatch(
                            setSelectedScheduleContent({
                                ...elt,
                            ...
                            buttons: [
                                {
                                    ...
                                    onClick: async () => {
                                        ...
                                        dispatch(setCurrentDateTime(moment().utc(true).format("YYYY-MM-DD")));
                                        dispatch(setIsShowFinalStep(true));
                                        dispatch(setShowSourceIdeasPopup(false));
                                        history.push("/lich-dang-bai");
                                    },
    ...
    const footer = (
        <button onClick={() => ds.current.load()} className="bg-primary text-white px-4 py-2 rounded-md">
            Xem thêm
        </button>
    );
    ...
    useEffect(() => {
        if (searchChannel) {
            const { channels = [], next_offset = 0, search_id = "", next_token } = searchChannel;
    ...
    const getMoreVideos = useCallback(() => {
        ...
        switch (type) {
            case "channel":
                dispatch(actionGetThreadsVideosByChannel(videoType?.channel_id, searchVideos?.next_cursor));
                break;

            case "collection":
                dispatch(actionGetThreadsVideosByCollection(searchVideos?.id, searchVideos?.next_cursor));
                break;
                ...
                <div className="list mt-3">
                    ...
                    {isShowChannels && !isFilter ? (
                        channels.length > 0 ? (
                            ...
                            <div className="text-base text-center p-4">
                                {isShowChannels && <span>Vui lòng chọn một kênh hoặc tìm từ khoá ở trên</span>}
                            </div>
                        )
                    ) : (
                        <></>
                    ...
                    {isFilter ? (
                        <Fragment>
                            ...
                            {isArrayEmpty(filterData) ? (
                                <div className="text-base text-center p-4">Không tìm thấy dữ liệu nào</div>
                            ) : (
