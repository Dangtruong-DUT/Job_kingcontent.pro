...
import ModalOverlay from "@/components/CategoriesContent/CategonesContentltem/ModalOverlay";
...
const ModalCollection = () => {
        ...
        switch (modalCollectionType) {
            ...
            case "addVideo":
                            ...
                            buttons: [
                                {
                                    ...
                                    onClick: async () => {
                                        setIsLoading(true);
                                        await dispatch(
                                            actionSaveVideoToCollection(selectedCollection?.value, "", chosenVideos)
                                        );
                ...
                if (collectionName) {
                            ...
                            buttons: [
                                {
                                    ...
                                    onClick: async () => {
                                        setIsLoading(true);
                                        await dispatch(actionSaveVideoToCollection(0, collectionName, chosenVideos));
                                        dispatch(actionUpdateChosenVideos([]));
                                        setIsLoading(false);
                    ...
                    await dispatch(actionSaveVideoToCollection(selectedCollection?.value, "", chosenVideos));
                    // clear chosen videos
                    dispatch(actionUpdateChosenVideos([]));
                    setIsLoading(false);
                    toast.success("Lưu video vào BST thành công");
    ...
    }, [modalCollectionType, collectionName, currentCollection, selectedCollection, chosenVideos, dispatch, onClose]);

    return (
        <Transition appear show={modalCollectionOpen} as={Fragment}>
            <Dialog as="div" className="relative z-9999 max-w-md mt-1" style={{ maxWidth: "30%" }} onClose={onClose}>
                <Transition.Child
                    as={Fragment}
                            ...
                            <Dialog.Panel className="w-30% transform rounded-lg pb-3 bg-white text-left align-middle shadow-xl transition-all relative">
                                {isLoading && (
                                    <div className="loading absolute w-full h-full top-0 bottom-0 right-0 left-0 flex justify-center items-center bg-gray-300 bg-opacity-50 z-10">
                                        <FaSpinner size={32} color="primary" className="animate-spin" />
                                    </div>
                                )}
                                ...
                                <div className="px-3">
                                    ...
                                    <div className="mt-4">
                                        <label className="block font-bold mb-2">
                                            {isShowSelection ? "Thêm mới bộ sưu tập" : "Tên bộ sưu tập"}
                                        </label>
                                        <input
                                            type="text"
                                        ...
                                        {isShowSelection && (
                                            <div className="mt-2">
                                                <label className="block font-bold mb-2">Hoặc chọn BST đã có</label>
                                                <div className="mt-4">
                                                    <Select
                                                        options={colOptions}
                                    ...
                                    <div className="flex justify-end mt-4 px-3">
                                        <Button color="blue" className="mr-2" onClick={() => onSave()}>
                                            Lưu
                                        </Button>
