...
const RightPart = (props) => {
    const { isSchedule = false, isAuto = false, handleAddToWaitingList = () => {}, leftOpen = false } = props;
    ...
    const handleAction = async (action, elt, collId = 0) => {
        switch (action) {
            case "VIEW_DETAIL_CONTENT":
                const { images = [], videos = [], feed_name = "", timestamp, duration = 0 } = elt;
            ...
            case "CHOOSE_VIDEO":
                const index = chosenVideos.findIndex((item) => item.video_id === elt.video_id);
                if (index > -1) {
                    const newChosenVideos = chosenVideos.filter((item) => item.video_id !== elt.video_id);
                    ...
                    buttons: [
                        {
                            label: "Chắc chắn",
                            onClick: async () => {
                                await dispatch(actionRemoveVideoFromCollection(collectionId, elt.video_id));
            ...
            case "SCHEDULE_CONTENT":
                if (elt.duration <= 300) {
                    ...
                    confirmAlert({
                        title: "Thông báo",
                        message: "Kingcontent chỉ hỗ trợ đăng video dài tối đa 5 phút. Vui lòng chọn video khác",
                        buttons: [
                            {
                                label: "Đồng ý",
                                onClick: () => {},
                    ...
                    buttons: [
                        {
                            ...
                            onClick: () => {
                                ...
                                const editorLink = document.createElement("a");
                                const baseEditorUrl = VIDEO_EDITOR_URL ?? "http://localhost:3000";
                                editorLink.href = `${baseEditorUrl}?type=douyin&id=${
                                    elt.video_id
                                }&thumbnail=${encodeURIComponent(elt.thumbnail)}`;
                                editorLink.target = "_blank";
                                document.body.appendChild(editorLink);
                                editorLink.click();
                                document.body.removeChild(editorLink);
    ...
    useEffect(() => {
        if (searchChannel) {
            const { users = [], next_cursor = 0, next_token } = searchChannel;
    ...
    const checkVideoIsChosen = useCallback(
        (video) => {
            const index = chosenVideos.findIndex((item) => item.video_id === video.video_id);
            if (index > -1) {
                return true;
            }
            return false;
    ...
    const selectAllVideos = useCallback(() => {
        ...
        const newChosenVideos = videos.filter((video) => {
            const index = collectionsVideos.findIndex((item) => item.video_id === video.video_id);
            if (index > -1) {
                return false;
            }
            return true;
    ...
    const getMoreVideos = () => {
        ...
        switch (type) {
            ...
            case "collection":
                dispatch(actionGetDouyinVideosByCollection(searchVideos?.id, parseInt(searchVideos?.page) + 1));
                break;

            default:
                const lastIds = convertListVideosToIds(searchVideos?.videos);
                dispatch(actionGetDouyinVideos(keyword, videoNextOffset, videoSearchId, nexToken, lastIds));
                break;
        }
    };
    ...
    return (
        <div className="results border border-gray-300 rounded-md bg-white p-2 border-b">
            {isLoading && isArrayEmpty(videos) ? (
                ...
                <div className="text-base text-center p-4">Không tìm thấy kết quả phù hơp</div>
            ) : (
                <>
                                                        ...
                                                        buttons: [
                                                            {
                                                                ...
                                                                onClick: () => {
                                                                    ...
                                                                    if (validVideos.length > 0) {
                                                                        ...
                                                                        const editorLink = document.createElement("a");
                                                                        const baseEditorUrl =
                                                                            VIDEO_EDITOR_URL ?? "http://localhost:3000";
                    ...
                    <div className="list mt-3">
                        {isShowChannels ? (
                            channels.length > 0 ? (
                                ...
                                <div className="text-base text-center p-4">Không tìm thấy kết quả phù hợp</div>
                            )
                        ) : (
                            <></>
                        )}
