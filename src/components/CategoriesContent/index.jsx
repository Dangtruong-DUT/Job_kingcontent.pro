...
function Contents(props) {
    ...
    const handleSearch = async (newQuery, page = 1) => {
        try {
            ...
            const res = await Client.get(`/categories/${id}/contents${query}&_limit=12&page=${page}`);
            if (res.status === OK) {
        ...
        <div className="rounded-lg">
            ...
            <div className="main">
                ...
                <div className="mt-4 mb-5 py-1 bg-blue-300 text-gray-800 text-center  text-sm uppercase font-bold drop-shadow-md flex items-center justify-center">
                    <LightBulbIcon className="w-6 h-6 text-yellow-100 mr-1" />Ý tưởng của bạn hôm nay
                </div>
                {contents.loading && (
                ...
                <div className="contentContainer relative">{!contents.loading && renderContents()}</div>
            </div>
        </div>
    );
}
