...
import Medias from "@/components/CategoriesContent/ContentDetail/Medias";
import Header from "@/components/CategoriesContent/ContentDetail/Header";
import Footer from "@/components/CategoriesContent/ContentDetail/Footer";
...
function ContentDetail() {
    ...
    const handelWritePost = () => {
        ...
        dispatch(resetCreateContent());
        dispatch(actionPushContentToCreateContentScreen(postText, images, media_type, wishlist === 1));
    ...
    return (
        <>
            {(!images && !videos) || ((images?.length === 0 || images[0] === "") && videos?.length === 0) ? (
                            ...
                            <div className="h-full w-full max-w-xl p-4 overflow-auto">
                                ...
                                {videogen_settings && (
                                    <div className="flex flex-col mb-4 bg-gray-100 p-4 rounded-md shadow-md">
                                        <div className="text-sm font-bold text-gray-700 mb-2">Cấu hình Video AI</div>
                                        <div className="text-sm text-gray-600 gap-1">
                                            <p>
                                                <strong>Tiêu đề: </strong>
                                                {videogen_settings.title}
                                            </p>
                                            <p>
                                                <strong>Chủ đề: </strong>
                                                {videogen_settings.prompt || "Tự soạn"}
                                                {". "}
                                                {videogen_settings.style && <strong>Phong cách: </strong>}
                                                {videogen_settings.style && videogen_settings.style + ". "}
                                            </p>
                                            <p>
                                                ...
                                                {". "}
                                                <strong>
                                                    {videogen_settings.caption_visible === 1 ? "Có" : "Không"}{" "}
                                                </strong>
                                                {"hiển thị phụ đề."}
                                            </p>
                                        </div>
                                ...
                                ></p>
                                {ad_id === "" && <Footer likes={likes} comments={comments} shares={shares} />}
                                {/* <div className="mt-2 flex justify-end">
                  {writed && (
                    <button
                      className=" bg-primary p-5 rounded-md text-white font-bold mr-2 hover:bg-gray-700 duration-300"
