...
const CategoriesContentItem = (props) => {
    ...
    const { cateId, content, page, setIsOpenPopupTag, setContentSelect, setIsMorePost } = props;
                    ...
                    buttons: [
                        {
                            label: "Có",
                            onClick: async () => {
                                const res = await Client.delete(`/liked-data/${content?.contentId}`);
                                if (res.status === OK) {
    ...
    return (
        <div className={`list-none align-top ${mb} w-full rounded-lg`} key={contentId}>
            <div className="bg-white rounded-t-lg  group relative contentItem" style={{ minHeight: "500px" }}>
                <Header
                ...
                <Image
                    medias={medias}
                    contentType={(videos && !isArrayEmpty(videos)) || media_type === "video" ? "video" : ""}
                />
                <ModalOverlay handleAction={handleAction} page={page} post_id={post_id} />
            </div>
            {page !== "ads" && (
                <Footer
                    likes={likes}
