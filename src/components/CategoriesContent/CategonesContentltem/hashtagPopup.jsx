...
import { REDUX_NAME_CONTENT_USER_LIKED, REDUX_NAME_CREATE_POST } from "@/utils/utilityFunc";
...
const PopupTag = ({ isOpen = true, toggle = () => {}, onSubmitClick, hashtag, setIsOpenPopupTag }) => {
    const dispatch = useDispatch();
    const { hashTags, contents } = useSelector((state) => state[REDUX_NAME_CONTENT_USER_LIKED]);
    ...
    const handelSaveEdit = (hashtag) => {
        const hashTagFilter = contents.filter((_elt) => _elt.hashtag === hashtagEdit).map((_hash) => _hash.id);
        axios
            .all(
    ...
    const handleDeleteHashtag = (hashtag) => {
        const hashTagFilter = contents.filter((_elt) => _elt.hashtag === hashtag).map((_hash) => _hash.type_id);
        confirmAlert({
            title: "Thông báo",
            message: "Khi xóa thẻ các bài viết có thể sẽ không được phân loại. Bạn có chắc muốn xóa không?",
            buttons: [
                {
                    label: "Có",
                    onClick: async () => {
    ...
    return (
        <Transition appear show={isOpen} as={Fragment}>
            <Dialog as="div" className="relative z-9999 max-w-lg mt-1" style={{ maxWidth: "50%" }} onClose={toggle}>
                <Transition.Child
                    as={Fragment}
                        ...
                        >
                            ...
                            >
                                <div className="mb-3">
                                    ...
                                    />
                                    <h4 className="font-bold text-base uppercase mt-2">Thẻ gợi ý</h4>
                                    <PerfectScrollbar className=" w-full mt-1" style={{ height: "200px" }}>
                                        <ul className="mt-2">
                                ...
                                <div className="flex justify-end gap-5 mt-3">
                                    ...
                                    <button
                                        onClick={() =>
                                            isEditMultil ? handelSaveEdit(inputValue) : onSubmitClick(inputValue)
                                        }
                                        disabled={inputValue === ""}
