...
function Image(props) {
    ...
    const renderMedias = () => {
        if (medias && medias.length > 0) {
            ...
            thumbnail = thumbnail && thumbnail.includes("instagram") ? convertInstagramLink(thumbnail) : thumbnail;
            ...
            const ImageElm = ({ ObjFit = "h-full", url, height }) => {
                return (
                    <img
                        ...
                        src={url && url.includes("instagram") ? convertInstagramLink(url) : url}
                        height={height}
                        alt=""
                        ...
                        <div className="w-full  h-hashtag-popup flex flex-row justify-between">
                            <div className="w-full flex flex-col justify-between overflow-hidden">
                                <div className="p-0.5 w-full h-full block relative">
                                    <ImageElm ObjFit="object-cover" url={medias[0]} style={{ height: "100%" }} />
                                    {isShowDelete && (
                                        <MdOutlineClear
                                            onClick={() => deleteImage(medias[0])}
                                            color="#fff"
                                ...
                                <div className="p-0.5 w-full h-full block relative">
                                    <ImageElm ObjFit="object-cover" url={medias[1]} style={{ height: "100%" }} />
                                    {isShowDelete && (
                                        <MdOutlineClear
                                            onClick={() => deleteImage(medias[1])}
                                            color="#fff"
                            ...
                            <div className="w-full flex flex-col justify-between overflow-hidden">
                                <div className="p-0.5 w-full block relative" style={{ height: "33.33333333333333%" }}>
                                    <ImageElm ObjFit="object-cover" url={medias[2]} />
                                    {isShowDelete && (
                                        <MdOutlineClear
                                            onClick={() => deleteImage(medias[2])}
                                ...
                                >
                                    ...
                                    {medias.length > 5 ? (
                                        <div className="absolute top-0 bg-gray-400 h-full w-full opacity-80 flex justify-center items-center">
                                            <span className="text-4xl text-white">+{numberImageResidual}</span>
                                        </div>
                                    ) : null}
                                </div>
                            </div>
    ...
    return (
        <div className="mt-8 w-full block relative cursor-pointer" onClick={() => handleActionShowPopup()}>
            {downloadedMediaUrl ? (
                <ReactPlayer
