...
const UserReels = (props) => {
    const { isSchedule = false, isAuto = false, handleAddToWaitingList = () => {} } = props;
    const {
        currentUser,
        reels = null,
        loadingReels = false,
                ...
                case 1: // scheduled
                    const filteredPosts = orderedReels.filter((post) => {
                        return scheduledContents.some(
                            (content) => content.content_id === post.id && content.source_type === "instagram"
                        );
                    });
    ...
    }, [reels, loadingReels, nextIsLoading, orderDir, orderType, scheduleFilter, scheduledContents]);

    const onLoadMore = () => {
        const { has_next = false, next_cursor = "" } = reels;
        if (has_next) {
            dispatch(actionGetUserReels(currentUser?.instagram_id || currentUser?.id, next_cursor));
        }
    };
    ...
    return (
        <div>
            {listReels.length > 0 ? (
                <div className={`grid gap-1 mt-3 ${isSchedule ? "grid-cols-3" : "grid-cols-3"}`}>
            ...
            {!loadingReels && !nextIsLoading && listReels.length > 0 && reels?.has_next && !currentHashTag && (
                <div className="text-center p-3 mt-3">
                    <button className="bg-blue-500 text-white px-5 py-3 rounded-md w-1/3" onClick={() => onLoadMore()}>
                        Lấy thêm bài viết
                    </button>
                </div>
            )}
