...
const RightPart = (props) => {
    const { isSchedule = false, isAuto = false, handleAddToWaitingList = () => {}, open = false } = props;
    ...
    useEffect(() => {
        if (!posts?.items?.length && !reels?.items?.length && !users?.length && !currentUser && !currentCollection) {
            setHasNoData(true);
        }
                                ...
                                {users.length > 0 ? (
                                    ...
                                    >
                                        {users.map((user, index) => (
                                            <SingleUser key={index} index={index} user={user} isSchedule={isSchedule} />
                                        ))}
                                    </div>
                                ) : (
                                ...
                                {!currentHashTag && (
                                    ...
                                    >
                                        ...
                                        <div
                                            ...
                                            style={{
                                                backgroundImage: `url(${convertInstagramLink(currentUser?.picture)})`,
                                            }}
                                        >
                                            {""}
                                        </div>
                                    ...
                                    <div className="collectionActions flex gap-2 items-center justify-center ml-auto bg-white">
                                        {chosenPosts.length > 0 ? (
                                            <div className="summary">
                                                ...
                                                <span className="font-bold">{chosenPosts?.length || 0}</span>/{" "}
                                                <span className="font-bold">{contentsData?.length || 0}</span>
                                                <span> bài viết</span>
                                            </div>
                                        ) : (
                                            <div className="summary hidden">
                                                <span>Chọn bài viết bên dưới để lưu vào bộ sưu tập</span>
                                            </div>
                                        )}
