...
const collectionScheduleAutoActions = [{ icon: ImEye, title: "Xem chi tiết", action: "VIEW_DETAIL_CONTENT" }];

const SinglePost = (props) => {
    const { post = {}, isSchedule = false, isAuto = false, handleAddToWaitingList = () => {} } = props;
    const {
        id = "",
        code = "",
        created = "",
    ...
    const { autoWaitingList, editingContents = [], scheduledContents = [] } = useSelector((state) => state.schedules);
        ...
        switch (action) {
            ...
            case "VIEW_DETAIL_CONTENT_COLLECTION":
                ...
                if (status === OK) {
                    ...
                    toast.error(data?.message || "Vui lòng liên hệ nhân viên để được hỗ trợ tốt hơn!");
                }
                toast.dismiss();
                break;
            ...
            case "CREATE_CONTENT_COLLECTION":
                            ...
                            },
                            {
                                ...
                                onClick: async () => {
                                    ...
                                    if (status === OK) {
                                        ...
                                        dispatch(
                                            actionPushContentToCreateContentScreen(text, medias, "video", is_reels)
                                        );
                                        history.push("/tao-content");
                                    } else {
                                        toast.error(
                ...
                const { images: images1 = [], is_reels: is_reels1, text: text1 } = data1?.data;
                if (status1 === OK) {
                    dispatch(createContentToHomepage({ status: true }));
                    dispatch(actionUpdateStep1(true));
                    dispatch(resetCreateContent());
    ...
    const handleAddToSchedule = (elt) => {
        dispatch(
            setSelectedScheduleContent({
                ...
                thumbnail: videos && videos.length > 0 ? videos[0].thumbnail : images[0],
            })
        );
        dispatch(setIsShowFinalStep(true));
        dispatch(setShowSourceIdeasPopup(false));
    ...
    useEffect(() => {
        if (editingContents && editingContents.length > 0 && isAuto) {
            const search = editingContents.find((elt) => elt.id === code && elt.source_type === "instagram");
            if (search) {
        ...
        <div key={id} className="singlePost">
            {isSchedule ? (
                <div className="relative flex gap-2 border border-gray-200 rounded-md cursor-pointer mb-2 p-2">
                    ...
                    {isScheduled && (
                        <span className="absolute bottom-3 right-1 text-red-500" title="Bài viết đã được lên lịch">
                            <FaExclamation />
                        </span>
                    )}
                    <div
                    ...
                    <div className="info w-2/3 p-2" onClick={() => handleAction("VIEW_DETAIL_CONTENT")}>
                        ...
                        <div className="created">
                            <span className="text-xs text-gray-500">Ngày tạo: {formatUnixDate(created)}</span>
                        </div>
                ...
                <div className="border border-gray-200 rounded-md cursor-pointer group relative flex mb-2 p-2">
                    ...
                    <div className="info w-2/3 p-2">
                        ...
                        <div className="desc">
                            <p className="text-sm font-medium whitespace-pre-wrap line-clamp-5">{text}</p>
                        </div>
                        <div className="created">
                            <span className="text-xs text-gray-500">Ngày tạo: {formatUnixDate(created)}</span>
                        </div>
                        ...
                        <div className="h-full w-full flex items-center">
                            <ul
                                className={`max-w-350 mx-auto ${
                                    defaultActions === actions ? "grid grid-cols-2" : "grid grid-cols-2"
                                }`}
                            >
                                {actions.map((item, index) => (
                                    ...
                                    >
                                        {item.action === "CHOOSE_POST" && isChosen ? (
                                            <FaCheckSquare color="green" className="w-6 h-6 text-gray-100" />
                                        ) : item.action === "FOLLOW_USER" && isSavedUser ? (
                                            <FaCheck color="green" className="w-6 h-6 text-gray-100" />
                                        ) : (
