...
const RightPart = (props) => {
    ...
    const handleAction = async (action, elt, collId = 0) => {
        switch (action) {
            ...
            case "CHOOSE_VIDEO": {
                const index = chosenVideos.findIndex((item) => item.post_id === elt.post_id);

                if (index > -1) {
                    const newChosenVideos = chosenVideos.filter((item) => item.post_id !== elt.post_id);
                    ...
                    buttons: [
                        {
                            label: "Chắc chắn",
                            onClick: async () => {
                                await dispatch(actionRemoveVideoFromCollection(collectionId, elt.post_id));
            ...
            case "EDIT_VIDEO": {
                ...
                if (duration > maxAllowDuration) {
                    confirmAlert({
                        title: "Thông báo",
                        message: `Hiện tại Kingcontent chỉ hỗ trợ chỉnh sửa video dài tối đa ${convertSecondsToReadableFormat(
                            maxAllowDuration
                        )}. Vui lòng nhấn nút tải video này về và upload trực tiếp lên trình chỉnh sửa video (tối đa 200MB)`,
                        buttons: [
                            {
                                label: "Tải về",
                                onClick: async () => {
                    ...
                    buttons: [
                        {
                            ...
                            onClick: () => {
                                ...
                                const editorLink = document.createElement("a");
                                const baseEditorUrl = VIDEO_EDITOR_URL ?? "http://localhost:3000";
                                editorLink.href = `${baseEditorUrl}?type=tiktok&id=${post_id}&thumbnail=${encodeURIComponent(
                                    elt.thumbnail
                                )}`;
                                editorLink.target = "_blank";
                                document.body.appendChild(editorLink);
            ...
            case "SET_SCRIPT_VIDEO_AI":
                {
                    ...
                    try {
                        ...
                        toast.error(
                            "Video này không có giọng đọc kịch bản hoặc giọng đọc kịch bản không rõ ràng, không thể lấy được kịch bản để tạo mới video AI."
                        );
                    } finally {
    ...
    const onFilteringVideos = () => {
        const { order = 0, orderType = 0, length = 0, videoType = 0 } = filteringSettings;
        ...
        switch (parseInt(videoType)) {
            case 1: // Reels format
                filterContents = filterContents.filter((item) => {
                    ...
                    if (height >= minHeight && width >= minWidth && ratio >= minRatio) return true;
                });
                break;
    ...
    useEffect(() => {
        if (searchVideos) {
            const { videos = [], next_offset = 0, search_id = "", page = 1, has_more = false } = searchVideos;
    ...
    const checkVideoIsChosen = useCallback(
        (video) => {
            const index = chosenVideos.findIndex((item) => item.post_id === video.post_id);
    ...
    const selectAllVideos = useCallback(() => {
        ...
        const newChosenVideos = videos.filter((video) => {
            const index = collectionsVideos.findIndex((item) => item.video_id === video.post_id);
    ...
    const getMoreVideos = () => {
        ...
        switch (type) {
            case "channel":
                dispatch(
                    actionGetTiktokVideosByChannel(videoType?.channel_id, videoNextOffset, currentChannelOrderType)
                );
                break;

            case "collection":
                dispatch(actionGetTiktokVideosByCollection(searchVideos?.id, parseInt(searchVideos?.page) + 1));
                break;

            default:
                dispatch(actionGetTiktokVideos(keyword, videoNextOffset, videoSearchId));
                break;
        }
    };
    ...
    const onOrderTypeChange = (orderType) => {
        ...
        dispatch(actionGetTiktokVideosByChannel(videoType?.channel_id, 0, orderType));
    };
                        ...
                        <div className="tabs flex gap-3 items-center">
                            <button
                                className={`tab_channel cursor-pointer p-3 ${
                                    isShowChannels ? "border rounded-md border-primary text-primary" : ""
                                }`}
                            ...
                            <button
                                className={`tab_video cursor-pointer p-3 ${
                                    isShowVideos ? "border rounded-md border-primary text-primary" : ""
                                }`}
                            ...
                            {isDevMode() && (
                                <button
                                    className={`tab_video cursor-pointer p-3 ${
                                        isShowMe ? "border rounded-md border-primary text-primary" : ""
                                    }`}
                            ...
                            <div className="flex gap-2 items-center justify-center ml-auto">
                                ...
                                <div className="actions flex gap-3 whitespace-nowrap">
                                    ...
                                    <button
                                        ...
                                        disabled={videos.length === 0 || chosenVideos.length === videos.length}
                                    >
                                        Chọn toàn bộ
                                    </button>
                                    <button
                                    ...
                                    {chosenVideos.length > 0 && auth.isHasVip3() && (
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                const invalidVideos = chosenVideos.filter(
                                                    (video) => video.duration > maxAllowDuration
                                                );
                                                ...
                                                if (invalidVideos.length > 0) {
                                                    confirmAlert({
                                                        ...
                                                        } video vượt quá thời lượng cho phép (${convertSecondsToReadableFormat(
                                                            maxAllowDuration
                                                        )}). Vui lòng bỏ chọn các video này trước khi chỉnh sửa tất cả.`,
                                                        buttons: [
                                                            {
                                                                label: "Bỏ video lỗi và tiếp tục",
                                                                onClick: () => {
                                                                    // Remove invalid videos and continue with valid ones
                                                                    const validVideos = chosenVideos.filter(
                                                                        (video) => video.duration <= maxAllowDuration
                                                                    );
                                                                    ...
                                                                    if (validVideos.length > 0) {
                                                                        ...
                                                                        const baseEditorUrl =
                                                                            env.VITE_VIDEO_EDITOR_URL ??
                                                                            "http://localhost:3000";
                                                                        const videoParams = validVideos
                                                                            .map(
                                                                                (video) =>
                                                                                    `type=tiktok&id=${
                                                                                        video.post_id
                                                                        ...
                                                                        toast.error(
                                                                            "Không có video hợp lệ để chỉnh sửa"
                                                                        );
                                                                    }
                                                                },
                                                            },
                                                ...
                                                const editorLink = document.createElement("a");
                                                const baseEditorUrl =
                                                    env.VITE_VIDEO_EDITOR_URL ?? "http://localhost:3000";
                                                const videoParams = chosenVideos
                                                    .map(
                                                        (video) =>
                                                            `type=tiktok&id=${
                                                                video.post_id
                        ...
                        {isShowVideos ? (
                            <div className="videoContainer">
                                ...
                                {videos.length > 0 ? (
                                    <div>
                                        <div
                                            className={`list_video grid grid-cols-2 lg:grid-cols-4 md:grid-cols-3 gap-2 ${
                                                leftOpen ? "" : "xl:grid-cols-5"
                                            }`}
                                        >
