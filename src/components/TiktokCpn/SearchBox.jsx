...
const SearchBox = (props) => {
    ...
    const { searchVideos = null, followingChannels = [], collections = [] } = useSelector((state) => state.tiktoks);
    ...
    return (
        <div className="relative z-30">
            <form className="flex gap-3 pt-3 mb-3" onSubmit={onSearch}>
                ...
                >
                    <img src={LogoTikTok} alt="icon-tiktok" className="w-7 rounded-full" />
                    <span className="text-xs">Thá»‹nh hÃ nh</span>
                </button>
                ...
                {!isFromManagePage && (
                    <Fragment>
                        ...
                        >
                            <img
                                src={IconTikTok}
                                alt="ThÃªm link thá»§ cÃ´ng"
                                title="ThÃªm link thá»§ cÃ´ng"
                                className="w-8"
                            />
                        </Button>
                        <Button
                            className="w-14 h-14 hover:text-white flex justify-center items-center rounded-lg hover:bg-primaryHover border"
                            title="Nguá»“n Ä‘Ã£ lÆ°u"
            ...
            {!isFromManagePage && (
                <div
                    className={`savedData transition-all ease-in-out gap-2 flex mb-4 ${
                        isShowSavedData ? "h-auto p-2 border rounded" : "h-0 overflow-hidden"
                    }`}
                >
                    <PerfectScrollbar className="folowingChannels w-1/2 max-h-64">
                        <Label className="font-bold uppercase">
                            Danh sÃ¡ch kÃªnh Ä‘Ã£ theo dÃµi ({followingChannels?.length || 0})
                        </Label>
                        {followingChannels &&
                            followingChannels.length > 0 &&
                            followingChannels.map((channel, index) => (
                                <Fragment key={index}>
                                    <SingleChannel channel={channel} isSchedule={!isFromManagePage} />
                                </Fragment>
                            ))}
                    </PerfectScrollbar>
                    <PerfectScrollbar className="collections w-1/2 max-h-64">
                        <Label className="font-bold uppercase">
                            Danh sÃ¡ch bá»™ sÆ°u táº­p Ä‘Ã£ lÆ°u ({collections?.length || 0})
                        </Label>
                        {collections &&
                            collections.length > 0 &&
                            collections.map((collection, index) => (
                                <Fragment key={index}>
                                    <SingleCollection collection={collection} isSchedule={!isFromManagePage} />
                                </Fragment>
                            ))}
                    </PerfectScrollbar>
                </div>
