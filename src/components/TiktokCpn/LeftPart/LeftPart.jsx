...
const LeftPart = () => {
    ...
    const { followingChannels = null, collections = null } = useSelector((state) => {
        ...
        if (followingChannels) {
            if (currentKeyword) {
                const filtered = followingChannels.filter(
                    (channel) =>
                        channel.feed_name.toLowerCase().includes(currentKeyword.toLowerCase()) ||
                        channel.display_name.toLowerCase().includes(currentKeyword.toLowerCase()) ||
                ...
                {isShowChannels ? (
                    <Fragment>
                        <div className="quickFilter p-1 mb-1">
                            <input
                                type="text"
                                placeholder="TÃ¬m kiáº¿m kÃªnh"
                                className="p-2 w-full rounded-md border border-gray-300"
                                onChange={() => onFilterChannel(event)}
                            />
                        </div>
                        {filteredChannels?.length > 0 ? (
                            ...
                            <h3 className="font-bold px-2">
                                Bạn chưa theo dõi kênh TikTok nào Hoặc không tìm thấy kênh Bạn
                            </h3>
                        )}
                    </Fragment>
                ) : null}
                ...
                {isShowCollections ? (
                    <div className="collectionsContainer">
                        ...
                        {collections?.length > 0 ? (
                            ...
                            <h3 className="font-bold px-2">Bạn có muốn tạo một BST mới không?</h3>
                        )}
                    </div>
                ) : (
                    <></>
