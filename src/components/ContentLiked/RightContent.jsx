            ...
            <div className="pt-3 float-left w-full" key={f._id}>
                <Link to={`/danh-muc/${f.category.slug}?fb_id=${f.fb_id}`}>
                    <div className="w-4/5 float-left">
                        <img src={baseURL + (f.avatar ? f.avatar.url : "")} className="w-20 h-20 float-left" />
                        <span className="font-bold pl-3 inline-block align-middle h-20 table-cell">{f.name}</span>
                    </div>
        ...
        <div className="w-2.4/10 mt-5 float-right">
            <div className="rounded-md p-3 bg-white shadow-smBlackShadow">
                <div className="title w-full">
                    <h2 className="text-base font-bold">Fanpage chá»©a content Ä‘Ã£ thÃ­ch</h2>
                </div>
