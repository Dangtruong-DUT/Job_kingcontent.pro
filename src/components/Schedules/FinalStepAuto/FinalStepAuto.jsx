...
const FinalStepAuto = (props) => {
    ...
    const onClickDestination = (destinationId, type) => {
        const findIndex = selectedDestinations.findIndex((item) => item.id === destinationId && item.type === type);
        if (findIndex > -1) {
            setSelectedDestinations(selectedDestinations.filter((item, index) => index !== findIndex));
        } else {
            switch (type) {
                case "fanpage":
    ...
    const onCheckSchedule = () => {
        ...
        const { contents = [], start_date = null, post_per_day = 0, source_type = "" } = localSettings || {};
        if (!start_date) {
            isError = true;
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        <div className="flex items-start justify-between px-2 py-4 border-b border-solid border-gray-300 rounded-t">
                            <div className="bg-gray-50  text-sm overflow-hidden pt-3 pb-3 flex1 justify-between border-l-4 border-green-500 pl-2 w-full">
                                <p className="font-bold uppercase text-base">Lên lịch đăng bài tự động</p>
                            </div>
                        </div>
                        {/*body*/}
                        <div
