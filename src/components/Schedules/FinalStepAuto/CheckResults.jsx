    ...
    try {
      ...
      userServices.addMultipleContents(postingData).then((res) => {
        ...
        if (success === false) {
          ...
          confirmAlert({
            title: 'ThÃ nh cÃ´ng',
            message: 'ÄÃ£ lÃªn lá»‹ch thÃ nh cÃ´ng, báº¡n cÃ³ muá»‘n lÃªn lá»‹ch tiáº¿p khÃ´ng?',
            buttons: [
              {
                label: 'Tiáº¿p tá»¥c',
                onClick: () => {
        ...
        <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          ...
          <div className="flex items-start justify-between px-2 py-4 border-b border-solid border-gray-300 rounded-t">
            <div className="bg-gray-50  text-sm overflow-hidden pt-3 pb-3 flex1 justify-between border-l-4 border-green-500 pl-2 w-full">
              <p className="font-bold uppercase text-base">Káº¿t quáº£ kiá»ƒm tra</p>
            </div>
          </div>
          {/*body*/}
              ...
              {result.map((item, idx) => {
                ...
                return (
                  <div className="resultItem w-full p-2" key={idx}>
                    BÃ i <strong>{contentId}</strong> sáº½ Ä‘Äƒng lÃºc{' '}
                    <strong>
                      {moment(date_publish).format('DD-MM-YYYY HH:mm:ss')}
                    </strong>{' '}
                    Ä‘áº¿n <strong>{total_destinations}</strong> kÃªnh Ä‘Ã£ chá»n
                  </div>
                );
              })}
            </div>
