...
import SingleDes from "./SingleDes.jsx";
import Filtering from "./Filtering.jsx";
import { FaAngleDown, FaAngleUp } from "react-icons/fa";
import { useHistory } from "react-router-dom";

const Destinations = (props) => {
    const { onClickDestination, selectedDestinations, reloadDestinations } = props;
        ...
        <div className="destinationContainer">
            {listDestinations && listDestinations.length === 0 ? (
                <p className="text-red-500 text-center text-base uppercase">
                    ÄÃ£ cÃ³ váº¥n Ä‘á» xáº£y ra khi káº¿t ná»‘i tá»›i Facebook, vui lÃ²ng{" "}
                    <span className="cursor-pointer underline font-bold italic" onClick={() => onClickGoToProfile()}>
                        kiá»ƒm tra láº¡i káº¿t ná»‘i táº¡i Ä‘Ã¢y
                    </span>
                </p>
            ) : (
                <Fragment>
                    <p className="text-blue-700 mb-3 cursor-pointer ml-3" onClick={() => reloadDestinations()}>
                        Làm mới danh sách
                    </p>
                    <div className="listFanpages m-2 overflow-hidden">
                        ...
                        >
                            <h4>
                                CHỌN NƠI ĐĂNG BÀI
                                <span className="text-blue-500"> ({filteredFanpages.length})</span>
                            </h4>
                            {isShowFanpages ? <FaAngleUp className="ml-auto" /> : <FaAngleDown className="ml-auto" />}
                        </div>
                        <div
                        ...
                        >
                                ...
                                <div className="grid gap-3 grid-cols-2">
                                    {threadsInfo && (
                                        <div className="threadsInfo mb-3">
                                            <h4 className=" text-sm font-bold uppercase p-3">Threads</h4>
                                    ...
                                    {tiktokInfo && (
                                        <div className="tiktokInfo mb-3">
                                            <h4 className=" text-sm font-bold uppercase p-3">TikTok</h4>
                                            <div className="w-full">
                                                <SingleDes
                            ...
                            <div className="listDestinations">
                                ...
                                <div className="filterFanpages">
                                    <Filtering type="fanpage" onFilterDestinations={onFilterDestinations} />
                                </div>
                                <div className="max-h-screen overflow-auto gap-3 grid grid-cols-2 border rounded-md p-1">
                                    {groupFanpagesByCategory &&
                                        Object.keys(groupFanpagesByCategory).map((category, idx) => {
                                            return (
                                                <div key={idx} className="pb-1">
                                                    <h4 className=" text-sm font-bold uppercase p-3">{category}</h4>
                                                    <div
                                                    ...
                                                    >
                                                        {groupFanpagesByCategory[category].map((item, idx) => {
                                                            return (
                                                                <Fragment key={idx}>
                                                                    <SingleDes
                                                                        destination={item}
