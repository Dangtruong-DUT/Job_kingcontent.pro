...
const buildListHours = (max) => {
    ...
    for (let index = 0.5; index <= max; index += 0.5) {
        ...
        const label = minutes === 0 ? `${hours} giờ` : `${hours > 0 ? hours + " giờ " : ""}${minutes} phút`;
        listHours.push({
            label: label,
...
const Settings = (props) => {
    ...
    useEffect(() => {
        if (sourceType) {
            ...
            if (sourceType === "user" && autoWaitingList?.contents) {
                const isReels = autoWaitingList?.contents?.every(
                    (content) => content?.is_reels === 1 || content?.is_reels === true
                );
                if (isReels) {
                    canBeReels = true;
                } else {
    ...
    useEffect(() => {
        if (autoWaitingList?.contents?.length > 0 && postPerDay && startDate && startTime) {
    ...
    return (
        <div className="settingsContainer overflow-hidden text-black  text-sm">
            ...
            >
                ...
                {isShowBody ? <FaAngleUp className="ml-auto" /> : <FaAngleDown className="ml-auto" />}
            </div>
            <div
            ...
            >
                <div className="summary my-2">
                    ...
                    <span className="font-bold">{autoWaitingList?.contents?.length || 0}</span>
                </div>
                {/* ROW day range */}
                <div className="settingRow my-2 flex gap-2 items-center">
                    ...
                    <div className="w-2/3 my-2 grid grid-cols-2">
                        ...
                        <div className="flex gap-2 items-center flex-nowrap">
                            <Label className="whitespace-nowrap">Thời gian bắt đầu đăng mỗi ngày:</Label>
                            <DatePicker
                                format="HH:mm"
                                ranges={[]}
                                style={{ width: 160 }}
                ...
                <div className="settingRow my-2">
                    <div className="flex gap-2 flex-nowrap items-center">
                        ...
                        <p className="notice text-red-500 w-1/3 ml-auto">
                            Khuyến cáo: Chỉ nên đăng 3-4 bài / ngày, mỗi bài cách nhau 3-4 giờ để có hiệu quả tốt nhất
                        </p>
                    </div>
                </div>
                ...
                <div className="rowIsAddSource settingRow my-2 flex gap-2 items-center">
                    <h5 className="font-bold w-1/3">Chọn để tự động thêm Nguồn bài viết ở cuối</h5>
                    <div className="w-2/3 grid items-center">
                        <Checkbox
                            ...
                            defaultChecked={true}
                            // @ts-ignore
                            onChange={(value, checked) => onChangeRandom("isAddSource", checked)}
                        >
                            Kích hoạt
                        </Checkbox>
                    </div>
                ...
                {canBeReels && (
                    <div className="rowIsReels settingRow my-2 flex gap-2 items-center">
                        <h5 className="font-bold w-1/3">Hình thức đăng: (chỉ dành cho bài viết dạng video)</h5>
                        <RadioGroup
                            name="isReels"
                ...
                <div className="settingRow my-2 flex gap-2 items-center">
                    <h5 className="font-bold w-1/3">Thêm ngẫu nhiên vào cuối bài viết:</h5>
                    <div className="w-1/3 grid grid-cols-2 items-center">
                        <Checkbox
                            ...
                            defaultChecked={true}
                            // @ts-ignore
                            onChange={(value, checked) => onChangeRandom("character", checked)}
                        >
                            3-5 Ký tự
                        </Checkbox>
                        <Checkbox
                ...
                {/* ROW before content */}
                <div className="settingRow my-2 flex gap-2 items-center">
                    <h5 className="font-bold w-1/3">Thêm nội dung vào đầu của toàn bộ bài viết:</h5>
                ...
                {/* ROW after content */}
                <div className="settingRow my-2 flex gap-2 items-center">
                    <h5 className="font-bold w-1/3">Thêm nội dung vào cuối của toàn bộ bài viết:</h5>
                    <div className="w-2/3 grid grid-cols-2 items-center">
                        <Input
                            componentClass="textarea"
                            rows={3}
                ...
                <div className="settingRow my-2 flex gap-2">
                    <div className="w-1/3">
                        <h5>
                            <span className="font-bold">Tìm kiếm và thay thế:</span> (Thay thế từ có sẵn ở bài viết gốc
                            thành từ mới của bạn)
                        </h5>
                        <p className="italic">
                            Ví dụ: Bài viết gốc có SĐT là A, bạn có thể sửa tự động thành B trên hàng loạt bài viết gốc
                        </p>
                    </div>
                    ...
                    <div className="inputs w-1/3">
                        ...
                        />
                        <Button className="bg-blue-700 text-white rounded-lg p-2" onClick={onAddSearchReplace}>
                            Thêm vào danh sách
                        </Button>
                    </div>
                    {listSearchReplace && listSearchReplace.length > 0 && (
                        <div className="results w-1/3 max-h-24 overflow-y-auto">
                            ...
                            {listSearchReplace.map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex gap-2 flex-nowrap items-center">
                                        ...
                                        <span className="w-1/12 cursor-pointer text-center">
                                            <FiX size={25} color="red" onClick={() => onRemoveSearchReplace(idx)} />
                                        </span>
                                    </div>
                                );
                            })}
