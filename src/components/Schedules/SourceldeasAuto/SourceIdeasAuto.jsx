...
const SourceIdeasAuto = (props) => {
    ...
    const renderContentType = (titleType, handleAddToWaitingList) => {
        switch (titleType) {
            case SPECIAL:
                return <Specials isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case CREATED:
                return <Plans isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case LIKED:
                return <Liked isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case TIKTOK:
                return <Tiktok isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case INSTAGRAM:
                return <Instagram isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case SYSTEM:
                return <Takecare isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case EVENT:
                return <Events isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case THREADS:
                return <Threads isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case DOUYIN:
                return <Douyin isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;

            case CHATGPT:
                return <ChatGPT isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;
            case VIDEOGEN_AI:
                return (
                    <TabCompleted parent={ParentType.SourceIdeasAuto} handleAddToWaitingList={handleAddToWaitingList} />
                );

            case VIDEO_EDITOR:
                return <VideoEditor isAuto={true} handleAddToWaitingList={handleAddToWaitingList} />;
    ...
    const onConfirmSchedule = () => {
        ...
        if (totalSelected === 0) {
            ...
            if (source_type === "douyin") {
                ...
                const countDurationLargerThan5Min = contents.filter((elt) => elt.duration > 300).length;
                // if (countNotReels > 0) {
                    ...
                    confirmAlert({
                        ...
                        buttons: [
                            ...
                            },
                            {
                                ...
                                onClick: () => {
                                    const newContents = contents.filter((elt) => elt.duration <= 300);
                                    dispatch(
                                        setScheduleWaitingList({
                                            ...autoWaitingList,
                                            contents: newContents,
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        <div className="flex items-start justify-between px-2 py-4 border-b border-solid border-gray-300 rounded-t">
                            <div className="bg-gray-50  text-sm overflow-hidden pt-3 pb-3 flex1 justify-between border-l-4 border-green-500 pl-2 w-full">
                                ...
                                {titleType !== DEFAULT ? <span className="italic">{descriptionType}</span> : ""}
                            </div>
                        </div>
                        {/*body*/}
