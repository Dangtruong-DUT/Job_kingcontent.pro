...
const SingleContent = (props) => {
    ...
    const { manageScheduleContents = null } = useSelector((state) => state.schedules);
    ...
    const handleDelete = () => {
        confirmAlert({
            title: "Xóa bài đăng",
            message: "Bạn có chắc chắn muốn xóa bài viết? Dữ liệu đã xóa không thể khôi phục.",
            buttons: [
                {
                    label: "Đồng ý",
                    onClick: () => {
                        ...
                        const newContents = manageScheduleContents.filter((content) => content.id !== id);
                        dispatch({
                            type: MANAGE_GET_SCHEDULE_CONTENTS_SUCCESS,
                            payload: newContents,
                        });
            ...
            case 5:
                return (
                    <div className="flex gap-2 items-center justify-center">
                        <Link href={publish_url} target="_blank" className="text-base no-underline w-40">
    ...
    return (
        ...
        >
            ...
            <div className="w-3/12 text-left overflow-hidden">
                <div className="flex gap-2 items-center">
                    <div
                        ...
                        style={{
                            backgroundImage: `url(${thumbnail ? thumbnail : source_content?.medias[0]})`,
                        }}
                    >
                        {media_type === "video" && (
                        ...
                        {media_type === "" && source_content && source_content.media_type === "video" && (
                            <div className="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center z-10">
                                <div className="w-6 h-6 bg-gray-400 rounded-full opacity-50">
                                    <FiPlayCircle className="w-full h-full text-white" />
                                </div>
                    ...
                    <div className="w-2/3">
                        <p className="text-sm line-clamp-3">{replaced_post_text || source_content?.post_text}</p>
            ...
            <div className="w-2/12">{renderStatus(status, publish_url, error_count, reason)}</div>
            <div className="w-2/12">
                <a href={destinationUrl} target="_blank" className="">
                    <p>{type === "threads" ? "Threads" : type === "tiktok" ? "TikTok" : destination_name}</p>
                    <p>({destination_id})</p>
                </a>
            </div>
            <div className="w-1/12 flex items-center justify-center">
                {content.source_type === "user" ? (
                    ...
                    <a href={getScheduleSourceLink(content)} target="_blank" className="inline-block text-blue-500">
                        <FaEye size={20} />
                    </a>
                )}
            </div>
            <div className="w-1/12 flex items-center gap-2 justify-center flex-col">
                {sourceLogo ? (
                    isFb ? (
                        ...
                        <img src={sourceLogo} alt="source" className="w-6 h-6" title={sourceName} />
                    )
                ) : null}
