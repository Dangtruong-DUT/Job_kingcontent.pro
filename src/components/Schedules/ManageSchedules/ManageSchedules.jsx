...
const ManageSchedules = (props) => {
    const { schedules = null, manageCurrentSchedule = null } = useSelector((state) => state.schedules);
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        ...
                        const newSchedules = schedules.filter((item) => item.id !== schedule.id);
                        dispatch({
                        ...
                        const newSchedules = schedules.map((item) => {
                            if (item.id === schedule.id) {
                                ...
                                return {
                                    ...
                                    pending_contents_count: contents_count - success_contents_count,
                                };
                            }
                            return item;
                        });
                        ...
                        const newSchedules = schedules.map((item) => {
                            if (item.id === schedule.id) {
                                ...
                                return {
                                    ...
                                    paused_contents_count: contents_count - success_contents_count,
                                };
                            }
                            return item;
                        });
    ...
    const onCheckSchedule = (schedule) => {
        const index = selectedSchedules.findIndex((item) => item.id === schedule.id);
        if (index === -1) {
            setSelectedSchedules([...selectedSchedules, schedule]);
        } else {
            setSelectedSchedules(selectedSchedules.filter((item) => item.id !== schedule.id));
        }
    };
    ...
    return (
        <div className="ManageSchedulesContainer fixed left-0 top-0 z-9999 w-full h-screen">
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none max-h-screen">
                <div className="relative w-98% mx-auto" style={{ maxHeight: "calc(100vh - 40px)" }}>
                    {/*content*/}
                            ...
                            <div className="bg-gray-50  text-sm overflow-hidden pt-3 pb-3 flex1 justify-between border-l-4 border-green-500 pl-2 w-full">
                                {manageCurrentSchedule ? (
                                    <p className="font-bold uppercase text-base">
                                        ...
                                        <span className="font-italic">{manageCurrentSchedule?.name}</span>
                                    </p>
                                ) : (
                                    <p className="font-bold uppercase text-base">Danh sách lịch đã lên</p>
                                )}
                            </div>
                        </div>
                        {/*body*/}
                                ...
                                <div className="w-full">
                                    ...
                                    {filteredSchedules?.length === 0 ? (
                                        <div className="flex justify-center items-center h-32 border rounded-md w-full pr-2">
                                            <p>Không có lịch nào phù hợp với từ khoá hoặc chưa có lịch nào được tạo</p>
                                        </div>
                                    ) : (
                                        <div
                                                            ...
                                                            <div className="w-2/3 flex gap-2 items-center">
                                                                ...
                                                                <div className="w-1/3 actions flex items-center gap-1 justify-center">
                                                                    ...
                                                                    <img
                                                                        ...
                                                                        title="Xoá lịch"
                                                                        className="w-6 h-6 cursor-pointer"
                                                                        onClick={() => onClickRemoveSchedule(schedule)}
                                                                    />
                                                                </div>
                                                            </div>
                                                        </div>
