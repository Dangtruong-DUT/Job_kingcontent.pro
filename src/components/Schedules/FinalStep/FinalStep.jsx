...
const FinalStep = (props) => {
    ...
    const [isRandomCharactersComment, setIsRandomCharactersComment] = useState(false);
    ...
    useEffect(() => {
        if (selectedDestinations && selectedDestinations.length > 0) {
            const hasThreads = selectedDestinations.find((item) => item.type === "threads");
    ...
    useEffect(() => {
        setHasMaxThreadsMedias(selectedScheduleContent?.medias?.length > 20 || false);
    }, [selectedScheduleContent]);
    ...
    const onClickDestination = useCallback(
        (destinationId, type) => {
            const searchResult = selectedDestinations.filter((item) => item.id === destinationId);
            if (searchResult.length) {
                setSelectedDestinations(selectedDestinations.filter((item) => item.id !== destinationId));
            } else {
                setSelectedDestinations([...selectedDestinations, { id: destinationId, type: type }]);
            }
        },
        [selectedDestinations]
    );
    ...
    useEffect(() => {
        if (selectedScheduleContent) {
            ...
            if (replaceContent) replaceContent = replaceContent.replace(/<br\s*\\?>/g, "\r\n");
    ...
    const onConfirmSchedule = () => {
        ...
        if (moment(selectedDateTime).isBefore(moment().add(5, "minutes"))) {
            isError = true;
            message = "Thời gian đăng bài phải lớn hơn thời gian hiện tại ít nhất 5 phút";
        }
        if (isError) {
            toast.error(message);
        } else {
            ...
            if (currentEditingContent) {
                ...
                destructedData = destructScheduleContent({
                    ...
                    scheduleName,
                    selectedDestinations,
                    selectedDateTime: moment(selectedDateTime).format("YYYY-MM-DD HH:mm:ss"),
                    replaceContent,
                    isReels,
                    isAddSource,
                    isAutoComment,
                    ...
                    {isDisableSubmit && (
                        <div className="absolute h-full w-full z-10 flex justify-center items-center bg-opacity-50 bg-gray-700">
                            <div className="relative">
                                ...
                                <label className="absolute top-1/3 z-10 w-0 h-0 overflow-hidden">Loading ...</label>
                            </div>
                        </div>
                    )}
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        <div className="flex items-start justify-between px-2 py-4 border-b border-solid border-gray-300 rounded-t">
                            <div className="bg-gray-50  text-sm overflow-hidden pt-3 pb-3 flex1 justify-between border-l-4 border-green-500 pl-2 w-full">
                                <p className="font-bold uppercase text-base">Lên lịch đăng bài</p>
                            </div>
                        </div>
                        {/*body*/}
                        <div
                            ...
                            <div className="w-full border-t p-4">
                                {hasMaxThreadsMessage ? (
                                    <p className="text-red-500 italic py-1 flex gap-2 font-bold items-center pl-2">
                                        <FiX className="" />
                                        <span>
                                            Content Threads của bạn đang vượt quá 500 ký tự, hệ thống sẽ tự động cắt bớt
                                            nội dung
                                        </span>
                                    </p>
                                ) : null}
                                {hasMaxThreadsMedias ? (
                                    <p className="text-red-500 italic py-1 flex gap-2 font-bold items-center pl-2">
                                        <FiX className="" />
                                        <span>
                                            Content Threads của bạn đang vượt quá 20 bức ảnh, hệ thống sẽ tự động lấy 20
                                            bức ảnh đầu tiên để đăng
                                        </span>
                                    </p>
                                ) : null}
                            </div>
