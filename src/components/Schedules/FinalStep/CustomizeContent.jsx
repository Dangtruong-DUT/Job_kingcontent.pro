...
const CustomizeContent = (props) => {
    ...
    return (
        <div className="customizeContentContainer overflow-hidden">
            ...
            <div className={`body transition-all duration-300 ease-in-out ${isShowBody ? "h-auto" : "h-0"}`}>
                <div className="flex gap-2">
                    <div className="border rounded-md w-4/6 p-3">
                        ...
                        {!listSourceTypeIgnoreSource.includes(selectedScheduleContent?.source_type) ? (
                            <div>
                                <Checkbox
                                    name="isAddSource"
                                    value={1}
                                ...
                                {selectedScheduleContent?.source_type === "video_ai" &&
                                    !selectedScheduleContent?.is_reels && (
                                        <span className="text-xs text-gray-400 ml-2 italic">
                                            (Video này chỉ đăng được ở chế độ thường, video Reels phải đạt điều kiện
                                            kích thước 9:16 và tối đa 90s)
                                        </span>
                                    )}
                            </div>
                        ) : null}
                        ...
                        {selectedScheduleContent?.source_type === "user" && false ? (
                            ...
                            >
                                Chọn để tự động thêm màu nền (Chỉ áp dụng cho bài viết không có ảnh / video)
                            </Checkbox>
                        ) : null}
                    </div>
