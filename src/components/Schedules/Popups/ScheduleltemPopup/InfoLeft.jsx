...
function InfoLeft(props) {
    ...
    const renderImage = () => {
        if (thumbnail) {
            return (
                <ImageElm
                    ObjFit="object-cover"
                    url={thumbnail.includes("instagram") ? convertInstagramLink(thumbnail) : thumbnail}
                />
            );
        } else {
    ...
    return (
        <div className="outline-none border-0 p-2 mx-1 flex flex-col">
            <div
                className={`relative cursor-pointer w-52 ${isMultiple ? "h-28" : "h-52"}`}
            ...
            >
                ...
                {isLoadingThumbnail === true && (
                    <div className={`w-52 flex items-center justify-center ${isMultiple ? "h-28" : "h-52"}`}>
                        Đang tải ảnh...
                    </div>
                )}
                {isVideo && (
                ...
                {images && images.length > 1 && (
                    <Fragment>
                        <div className="absolute top-1/2 left-1/2 bg-gray-600 opacity-70 rounded-full w-12 h-12 flex items-center justify-center transform -translate-x-1/2 -translate-y-1/2">
                            <span className="text-white font-medium text-base">+ {images?.length - 1}</span>
                        </div>
            ...
            <Button isSuccess={status === 1 || status === 5} scheduleContent={scheduleContent} />
        </div>
    );
}
