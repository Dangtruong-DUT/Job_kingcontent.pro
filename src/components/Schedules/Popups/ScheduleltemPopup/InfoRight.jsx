    ...
    return (
        <div className="flex flex-col p-2 mx-1 w-full relative">
            <div className=" text-sm font-bold mr-1 gap-2 items-center hidden">
                <span>Nơi đăng:</span>
                <Link href={`https://facebook.com/${destination_id}`} target="_blank" className="text-base">
                    {destination_id}
                </Link>
            ...
            ></TextStyle>
            {loading && <p className="italic text-blue-500">Đang lấy dữ liệu thống kê, vui lòng chờ trong giây lát</p>}
            {!loading && (status === 1 || status === 5) && (
                <div className="flex items-center mt-auto">
                    <div className="flex items-center mr-5">
                        <AiFillLike className="h-5 w-5 text-gray-500" />
                        <span className="ml-1  text-sm text-gray-800 font-bold">{kFormatter(likes)}</span>
                    </div>
                    <div className="flex items-center mr-5">
                        <FaCommentDots className="h-5 w-5 text-gray-500" />
                        <span className="ml-1  text-sm text-gray-800 font-bold">{kFormatter(comments)}</span>
                    </div>
                    <div className="flex items-center mr-5">
                        <FaShare className="h-5 w-5 text-gray-500" />
                        <span className="ml-1  text-sm text-gray-800 font-bold">{kFormatter(shares)}</span>
                    </div>
                </div>
            )}
