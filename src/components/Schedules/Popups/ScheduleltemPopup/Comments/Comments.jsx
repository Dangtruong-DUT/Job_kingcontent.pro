...
const Comments = (props) => {
    ...
    const [listScheduleComments, setListScheduleComments] = useState(scheduleComments);
    const {
        valueMonday,
        valueSunday,
            ...
            .then(
                ...
                (res) => {
                    ...
                    if (success === false) {
                        ...
                        const currComment = listScheduleComments.find((item, idx) => idx === index);
                        if (currComment) {
    ...
    const renderResult = (id, commentType = "", commentUrl = "") => {
        ...
        if (id) {
            switch (localType) {
                case "fanpage":
                    return (
                        <a href={`https://fb.com/${id}`} target="_blank" className="text-blue-500 text-center block">
                ...
                case "threads":
                    return (
                        <a href={localUrl} target="_blank" className="text-blue-500 text-center block">
                            <FiEye size={20} className="inline-block" />
                        </a>
                    );
                default:
    ...
    const renderActions = (index, fromAuto, published_id) => {
        ...
        if (published_id) {
            return <div className="w-2/12 flex gap-2 justify-center items-center">-</div>;
        }
        return (
            <div className="w-2/12 flex gap-2 justify-center items-center">
                ...
                <FiTrash
                    ...
                    title={"Xoá Comment"}
                    className="cursor-pointer"
                    onClick={() => onRemoveComment(index, listScheduleComments[index]?.id)}
                />
            </div>
        );
    };
        ...
        <div className="flex gap-2">
            {!fromAuto && (
                <div className="w-1/4 border rounded-md p-3">
                    <h4 className="font-bold mb-2">Loại tự động lên sau khi bài viết lên thành công</h4>
            ...
            <div className={`${fromAuto ? "w-full" : "w-3/4"} border rounded-md p-3`}>
                <h4 className="font-bold mb-2">
                    {fromAuto ? "Danh sách comment đã lên lịch" : "Loại lên lịch theo ngày"}
                </h4>
                ...
                {is_posted === false && (
                    <p className="font-bold text-red-500">
                        Bài viết chưa được đăng thành công hoặc chưa tới giờ đăng bài, vui lòng quay lại sau.
                    </p>
                )}
                {is_posted === true &&
                                    ...
                                    <div className="flex gap-3 items-center">
                                        ...
                                        <div className="w-1/12 justify-center flex">
                                            <Checkbox
                                                ...
                                                value={1}
                                                defaultChecked={
                                                    is_random_characters === 1 || is_random_characters === true
                                                }
                                        ...
                                        <div className="w-1/12 justify-center flex">
                                            <Checkbox
                                                name="isRandomEmojis"
                                                value={1}
                                                defaultChecked={is_random_emojis === 1 || is_random_emojis === true}
                                                className="text-center"
                                                inline={true}
                                                onChange={(value, checked) => {
                                                    const newListComments = [...listScheduleComments];
                                ...
                                {!editable && (
                                    <div className="flex gap-3 items-center">
                                        ...
                                        <div className="w-1/12">
                                            {renderResult(published_id, commentType, commentUrl)}
                                        </div>
                ...
                {is_posted === true && listScheduleComments && listScheduleComments.length === 0 && (
