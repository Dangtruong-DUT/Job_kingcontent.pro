...
function SuggestionsPopup(props) {
    ...
    const showSuggestionsPopup = useSelector((state) => state.schedules.showSuggestionsPopup);
    // const sortBy = useSelector(state => state.schedules.sortBy);
    const currentSuggestionsSubject = useSelector((state) => state.schedules.currentSuggestionsSubject);
    ...
    useEffect(() => {
        ...
        if (currentSuggestionsSubject) {
            const { id = 0, keywords = [], name = "", total = 0, type = "", parent_id = 0 } = currentSuggestionsSubject;
            switch (type) {
                case "system":
                    if (parent_id) {
    ...
    return (
        <div className="min-w-screen h-screen fixed inset-0 z-9999 flex items-center justify-center">
            ...
            >
                ...
                />
                <Header />
                <SearchBar handleSortPanel={handleSortPanel} getQueryKeywords={getQueryKeywords} />
                <FilterAndSort
