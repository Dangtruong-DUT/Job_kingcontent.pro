...
const SingleContent = (props) => {
    ...
    const { autoWaitingList, scheduledContents, editingContents = [] } = useSelector((state) => state.schedules);
    ...
    useEffect(() => {
        if (autoWaitingList) {
            ...
            if (contents.length > 0) {
                const search = contents.find((elt) => elt && elt?.post_id === item?.post_id);
                if (search) {
                    setIsSelected(true);
                } else {
                    setIsSelected(false);
    ...
    useEffect(() => {
        if (scheduledContents && scheduledContents.length > 0) {
            const search = scheduledContents.find((elt) => {
                if (content_type === "user") return parseInt(elt?.content_id) === parseInt(item?.id);
    ...
    useEffect(() => {
        if (editingContents && editingContents.length > 0) {
            const search = editingContents.find((elt) => elt?.id === (item?.event_id > 0 ? item?.id : item?.post_id));
            if (search) {
        ...
        >
            ...
            <div className="w-8/12" onClick={() => handleActionShowPopup(item)}>
                ...
                <ContentStyled className="overflow-hidden">
                    <p className="line-clamp" dangerouslySetInnerHTML={{ __html: displayPostText }}></p>
                </ContentStyled>
                {content_type !== "user" ? (
                    ...
                    <div className="flex gap-2 items-center mt-3 italic text-xs">{item?.note}</div>
                )}
                ...
                {content_type === "liked" && hashtag && (
                    <div className="flex p-3 items-center mt-2 italic text-xs">
                        {hashtag.indexOf("#") === -1 && <span className="font-bold text-gray-500">#</span>}
                        <span>{hashtag}</span>
                    </div>
                )}
            </div>
            ...
            {isScheduled && (
                <span className="absolute bottom-1 right-1 text-red-500" title="Bài viết đã được lên lịch">
                    <FaExclamation />
                </span>
            )}
        </div>
