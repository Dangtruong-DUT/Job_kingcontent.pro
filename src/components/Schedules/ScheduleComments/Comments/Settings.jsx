...
const generateTimeOptions = () => {
    ...
    for (let index = 5; index <= 120; index += 5) {
        timeOptions.push({
            label: `${
                index >= 60
                    ? `${Math.floor(index / 60)} giờ ${index % 60 === 0 ? "" : (index % 60) + " phút"}`
                    : `${index} phút`
            }`,
            value: index,
        });
...
const Settings = () => {
    ...
    useEffect(() => {
        const isEmpty =
            ...
            (typeof scheduleCommentsWaitingList === "object" && Object.keys(scheduleCommentsWaitingList).length === 0);
        if (isEmpty) {
            ...
            setBeginDate(
                scheduleCommentsWaitingList.begin_date ? new Date(scheduleCommentsWaitingList.begin_date) : new Date()
            );
            setStartTime(
                scheduleCommentsWaitingList.start_time ? new Date(scheduleCommentsWaitingList.start_time) : new Date()
            );
            setMultipleTimeSpace(scheduleCommentsWaitingList.multiple_timespace || null);
        }
    }, [scheduleCommentsWaitingList]);
        ...
        <div className="settingsContainer mt-4 flex items-center gap-2">
            ...
            <div className="multipleTimeSpace flex flex-wrap items-center gap-2">
                ...
                <Select
                    ...
                    onChange={(selected) => onChangeSettings("multipleTimeSpace", selected ? selected.value : null)}
                    value={
                        multipleTimeSpace
