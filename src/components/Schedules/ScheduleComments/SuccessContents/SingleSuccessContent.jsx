...
const SingleSuccessContent = (props) => {
    const { content = null, onChooseContent, setIsShowCommented = () => {}, setListComments = () => {} } = props;
    const [thumbnail, setThumbnail] = useState(defaultImg);
    const [description, setDescription] = useState("");
    const [datePublish, setDatePublish] = useState("");
    const [hasComments, setHasComments] = useState(false);

    const { scheduleCommentsWaitingList } = useSelector((state) => state.schedules);
    ...
    return (
        <div className="single-success-content flex gap-4 border-b border-gray-400 border-dashed hover:border-blue-500 p-2 mb-2 items-center cursor-pointer">
            <div className="checkbox w-1/12" onClick={() => onChooseContent(content?.id)}>
            ...
            <div className="content w-7/12 xl:w-8/12">
                <div onClick={() => onChooseContent(content?.id)}>
