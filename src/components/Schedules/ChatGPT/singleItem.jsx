...
import { FiCheckCircle, FiEdit, FiEye, FiMaximize, FiPlusCircle, FiTrash } from "react-icons/fi";
...
const SingleItem = (props) => {
    ...
    const convertedTitle = question.replace(`${defaultThreadsPrompt} `, "Nội dung gốc: ");
    ...
    useEffect(() => {
        if (editingContents && editingContents.length > 0) {
            const search = editingContents.find((elt) => elt.id === item.id && elt.source_type === "chatgpt");
            if (search) {
        ...
        <div className="bg-gray-100 rounded-lg shadow-sm p-3 relative flex gap-2">
            ...
            <div className="w-11/12">
                ...
                <div className="flex justify-between gap-5 mt-3 items-center">
                    {tag ? (
                        <span className="bg-green-500 rounded-md text-white px-3 py-1 cursor-pointer">{tag}</span>
                    ) : null}
