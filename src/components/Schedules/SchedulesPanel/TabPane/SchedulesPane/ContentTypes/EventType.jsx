...
function EventType(props) {
    ...
    const { scheduleEvents, selectedEvent = null } = useSelector((state) => state.schedules);
    ...
    return (
        <li onClick={handleSelectSuggestsPopup} className="relative py-3 px-2 border cursor-pointer event-type">
            <div className="relative flex flex-col items-center justify-between rounded-lg p-2">
                <div className="relative mt-1 w-24 h-24 rounded-full overflow-hidden ring-0 outline-none flex items-center">
                    <img src={event?.event_icon} alt={event?.title || ""} className="w-full h-full object-cover" />
