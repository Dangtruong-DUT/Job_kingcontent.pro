...
function ContentType(props) {
    const { styleContent, contents = [], isEvent, hour, isMultiple = false, day } = props;
    ...
    useEffect(() => {
        if (contents && contents.length > 0) {
            ...
            if (isMultiple) {
                ...
                const successCount = contents.filter((content) => content.status === 1 || content.status === 5).length;
                if (successCount > 0) {
    ...
    useEffect(() => {
        if (currentContent) {
            const { source_content = null, source_type = "system", thumbnail = "", media_type = "" } = currentContent;
            switch (source_type) {
                case "tiktok":
                    setIsVideo(true);
    ...
    const handleSelectSuggestsPopup = useCallback(() => {
        const selectedDate = `${moment(day).format("YYYY-MM-DD")} ${moment(hour).format("HH:mm:ss")}`;
        ...
        if (moment(selectedDate) < time) {
            ...
            if (moment(time).isBetween(selectedDate, nextHour)) {
                ...
                const newSelectedDate = `${moment(day).format("YYYY-MM-DD")} ${moment(time)
                    .add(15, "minutes")
                    .format("HH:mm:ss")}`;
        ...
        <li className="cursor-pointer content-type border group relative py-3 px-2">
            ...
            >
                <div
                    ...
                    style={{
                        backgroundImage: `url(${
                            currentContent?.source_content?.preset?.url ||
                            currentContent?.source_content?.random_preset?.url
                        })`,
                    }}
                >
                    {isUserContent && isVideo ? (
                        ...
                        <div
                            className={`w-full line-clamp-3 ${
                                hasPreset ? "text-white h-12 flex items-center justify-center" : "h-full"
                            }`}
                        >
                            {currentContent?.replaced_post_text || currentContent?.source_content?.post_text}
                        </div>
                    )}
                    {isVideo && (
                ...
                {isMultiple && hasSuccess && (
                    <div className="mt-2 flex items-center gap-1">
                        ...
                        <span className="text-xs font-bold whitespace-nowrap">{statusMessage}</span>
                    </div>
                )}
                {isMultiple && !hasSuccess && (
                    <div className="mt-2 flex items-center gap-1">
                ...
                {!isMultiple && (
                    <div className="mt-2 flex items-center gap-1">
                        <styleContent.icon className={`h-5 w-5 rounded-full text-white ${styleContent.bgColor}`} />
                        <span className="text-xs font-bold whitespace-nowrap">{styleContent?.name || ""}</span>
                    </div>
                )}
            </div>
            {isEvent === true && (
            ...
            {isMultiple && (
                <div className="absolute top-0 right-0 bg-red-500 rounded-full w-6 h-6 mt-1 mr-1 text-center">
                    <span className="text-base font-bold text-white">{contents.length}</span>
                </div>
            )}
            {sourceLogo && (
                <img
