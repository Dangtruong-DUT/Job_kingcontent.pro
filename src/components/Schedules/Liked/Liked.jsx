...
const Liked = (props) => {
    ...
    const { likedContents: contents = null } = useSelector((state) => state.userReducer);
        ...
        if (currentKw || currentHashtag) {
            ...
            if (currentKw) {
                newFilteredContents = newFilteredContents.filter((item) => {
                    ...
                    if (content) {
                        return content.post_text.toLowerCase().includes(currentKw.toLowerCase());
                    }
                    return false;
                });
            }
