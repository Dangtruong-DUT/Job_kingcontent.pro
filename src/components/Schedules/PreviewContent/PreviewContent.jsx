...
const PreviewContent = (props) => {
    ...
    useEffect(() => {
        if ((videos && videos.length > 0) || source_type === "tiktok" || media_type === "video") {
    ...
    }, [videos, source_type, schedule_content_id, post_id, thumbnail, medias, ad_id, images, media_type, video]);
    ...
    const onChangeImage = async (e) => {
        ...
        if (res.status === OK) {
            let images = res.data.data.slice(0, filesArr.length).map((_elt) => _elt.url);
            dispatch(
    ...
    useEffect(() => {
        if (user && user.facebook_id) {
            setAvatar(`https://graph.facebook.com/${user?.facebook_id}/picture?width=1000&height=1000`);
        } else {
            setAvatar(logo);
        }
    }, [user, logo]);
        ...
        <div className="list-none align-top mb-5 w-2/6 rounded-lg previewContent">
            ...
            >
                <div className="bg-white rounded-t-lg relative contentItem">
                    ...
                    <div className="mt-3">
                        {listTypesHasUpload.includes(source_type) && media_type !== "video" && (
                            <label
                        ...
                        <Image
                            medias={
                                source_type === "tiktok" || media_type === "video"
                                    ? [thumb]
                                    : ad_id || source_type === "douyin" || source_type === "instagram"
                                    ? images
                                    : medias
                            }
            ...
            {contentDetailToShow && source_type === "threads" && <ContentDetailThreads />}
        </div>
    );
};
