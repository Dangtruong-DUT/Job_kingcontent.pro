...
const Plans = (props) => {
    ...
    const { autoWaitingList = [], userPlans = [] } = useSelector((state) => state.schedules);
    ...
    const onSelectPlan = (plan) => {
        ...
        const filterContents = contents.filter(
            (elt) =>
                elt.post_text && elt.post_text.length > 0 && elt.post_text.trim().toLowerCase() !== "admin_suggestion"
        );
    ...
    return (
        <div>
            {!isShowContents && (
                <div className="plans">
                    <h3 className="uppercase font-bold text-center text-base mb-1">Danh sách kế hoạch của bạn</h3>
                    <p className="italic text-center mb-2">Hãy chọn 1 kế hoạch bên dưới để xem bài viết</p>
                    {/* TOP BAR  */}
                    ...
                    {listPlans.length === 0 ? (
                        ...
                        <PerfectScrollbar className="grid gap-5 mt-2 grid-cols-4 " style={{ maxHeight: "90vh" }}>
                            {listPlans.map((elt, index) => {
                                const { id, name, thumbnail, hashtag, createdDate, color, contents_count } = elt;
