...
import SearchBox from "@/components/TiktokCpn/SearchBox.jsx";
import Card from "@/components/TiktokCpn/Card.jsx";
import React, { Fragment, useCallback, useEffect, useState } from "react";
import SingleTiktokContent from "../SingleContent/SingleTiktokContent.jsx";
import DetailTiktok from "./DetailTiktok.jsx";
import {
...
import { MODAL_TYPE } from "../../TiktokCpn/constants.js";
// import LoadingApp from "../../LoadingApp/index.jsx";
import SingleChannel from "../../TiktokCpn/SingleChannel.jsx";
import PerfectScrollbar from "react-perfect-scrollbar";
import Select from "react-select";

// Default keywords for TikTok search
const listDefaultKws = ["funny", "dance", "challenge", "music", "trend"];
...
const Tiktok = (props) => {
    ...
    const { autoWaitingList, scheduledContents = [] } = useSelector((state) => state.schedules);

    useEffect(() => {
        if (searchVideos?.videos?.length === 0 && !currentKeyword) {
            const searchValue = listDefaultKws[Math.floor(Math.random() * listDefaultKws.length)];
    ...
    const onFilteringVideos = () => {
        const { order = 0, orderType = 0, length = 0, videoType = 0, scheduledType = 0 } = filteringSettings;
            ...
            case 1: // scheduled
                filterContents = filterContents.filter((item) => {
                    const search = scheduledContents?.find((elt) => {
                        return elt?.content_id === item?.post_id && elt?.source_type === "tiktok";
                    });
    ...
    const getMoreVideos = () => {
        ...
        switch (type) {
            case "channel":
                dispatch(actionGetTiktokVideosByChannel(videoType?.channel_id, nextOffset, currentChannelOrderType));
                break;
        ...
        <div className="pr-5 pl-5">
            ...
            {filteredContents.length === 0 && channels.length === 0 && !isLoading ? (
                <div className="flex justify-center h-60 items-center">
                    <span className="font-bold">Nhập từ khoá để có được kết quả chính xác</span>
                </div>
            ) : isLoading ? (
                <LoadingApp />
            ) : (
                            ...
                            <PerfectScrollbar className="max-h-98">
                                <div className="list_channel grid grid-cols-2 gap-3">
                                    {channels?.map((channel) => (
                                        <SingleChannel key={channel.id} channel={channel} isSchedule={true} />
                                    ))}
                                </div>
                        ...
                        <Fragment>
                            <div
                                className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 ${
                                    isCreatedContent ? "" : "xl:grid-cols-3"
                                }  gap-2`}
                            >
                                {renderVideos()}
                            </div>
