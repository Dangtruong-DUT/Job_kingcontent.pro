        ...
        <div className="w-full py-0 px-5 my-0 mx-auto mb-5">
            <div className="w-full my-0 mx-auto block">
                <div className="w-full mx-auto flex items-center justify-between border border-solid border-gray-300 rounded-md p-3">
                    <label>ThÃªm vÃ o bÃ i viáº¿t</label>
                    <div className="flex items-center justify-start">
                        <PhotographIcon
                            className="h-8 w-8 transition-all cursor-pointer hover:opacity-60 text-createContent-greenClr"
                            onClick={() => ChangeStateGetMedia(true, !isGetImages, false, false)}
                        />
                        <VideoCameraIcon
                            className="h-8 w-8 transition-all cursor-pointer hover:opacity-60 text-createContent-purpleClr ml-1.5"
                            onClick={() => ChangeStateGetMedia(true, false, !isGetVideo, false)}
                        />
                        <EmojiHappyIcon
                            className="h-8 w-8 transition-all cursor-pointer hover:opacity-60 text-createContent-orangeClr ml-1.5"
                            onClick={() => ChangeStateGetMedia(true, false, false, !isGetEmoji)}
                        />
