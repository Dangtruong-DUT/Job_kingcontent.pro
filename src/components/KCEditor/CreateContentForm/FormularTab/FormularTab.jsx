...
const FormularTab = (props) => {
    const { suggestionContents, addContentToEditor = () => {} } = props;
            ...
            <div className="flex">
                ...
                <div className="w-9/12">
                    <ul className="flex ml-2 mt-2">
                        <li className="mr-2">
                            <button
                                className={`inline-flex items-center justify-center cursor-pointer p-2 border rounded border-solid border-gray-400 ${
                                    isParagraph ? "bg-blue-100" : "bg-white"
                                }`}
                                onClick={() => {
                                    showParagraph(true);
                                }}
                    ...
                    {isSearch && !loading && !isParagraph && (
                        <SuggestionContents contents={suggestionContents} addContentToEditor={addContentToEditor} />
                    )}
                </div>
            </div>
        </div>
