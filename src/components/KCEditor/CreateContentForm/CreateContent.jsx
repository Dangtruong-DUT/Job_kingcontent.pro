...
function CreateContent(props) {
    ...
    const { selectedCatId, createdContentSuccess, currentContentType } = useSelector((state) => state.editor);
    const { selectedKeyword, imagesContent } = useSelector((state) => state.contents);
    ...
    const addContentToEditor = useCallback(
        (data, replace = false) => {
            if (replace) {
                ...
                const newState = EditorState.push(editorState, tmp, "replace-all-content");
                ...
                if (selectionState.isCollapsed() == true) {
                    ...
                    const newState = EditorState.push(editorState, tmp, "insert-characters");
                    setEditorState(newState);
                } else {
                    //replace
                    const tmp = Modifier.replaceText(contentState, selectionState, data);
                    const newState = EditorState.push(editorState, tmp, "replace-characters");
                    setEditorState(newState);
                }
            }
        },
                ...
                <div className="flex">
                    <div className="w-2/5">
                        <div className="max-h-96 h-full p-5 bg-gray-50 mx-6 my-4 rounded-md  text-sm overflow-hidden">
                            <Input editorState={editorState} setEditorState={setEditorState} />
                        </div>
                        <Options
                            canSave={canSave}
                    ...
                    <div className="w-3/5 mx-6 my-4">
                        ...
                        <div className="tabsContent">
                            ...
                            {isShowFormular && <FormularTab addContentToEditor={addContentToEditor} />}
                        </div>
                    </div>
                </div>
            </div>
