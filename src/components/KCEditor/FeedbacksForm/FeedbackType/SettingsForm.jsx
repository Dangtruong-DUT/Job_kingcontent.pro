...
const SettingsForm = (props) => {
    ...
    useEffect(() => {
        if (feedbackSettings) {
            const { branchName, clock, battery, customerName, activity, avatar } = feedbackSettings;
    ...
    }, [branchNameVal, clockVal, batteryVal, customerNameVal, activityVal, avatarVal]);
        ...
        <div className="settingsForm z-9999 absolute top-0 left-0 right-0 h-screen bg-createContent-modalOverLayClr pt-28">
            <div className="mx-auto my-0 w-5/6 max-h-80 bg-white rounded-md shadow-md p-4">
                ...
                <div className="barStatus grid grid-cols-3 gap-3">
                    ...
                    <div className="flex items-center rounded-lg border border-gray-300">
                        <FontAwesomeIcon className="text-base w-10 mx-2" icon={faBatteryHalf} />
                        <input
                            type="number"
                            className="w-full border-0 bg-transparent text-center"
                            value={batteryVal}
...
export default connect(mapStateToProps, { changeFeedbackSettings })(SettingsForm);
