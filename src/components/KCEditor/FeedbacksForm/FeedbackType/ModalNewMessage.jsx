...
const ModalNewMessage = (props) => {
    const { type = "send", setIsShowModalMessage, messageData } = props;
            ...
            <div className="mx-auto my-0 w-5/6 max-h-80 bg-gray-200 rounded-md shadow-md text-center relative">
                <div className="text-right py-2 h-8 relative">
                    <XIcon
                        ...
                        onClick={() => setIsShowModalMessage(false)}
                    />
                </div>
                ...
                <div className="w-full py-3 outline-none border-t border-gray-400 text-center">
                    {type === "send" && (
                        <button
                            className="p-3 bg-createContent-blueClr text-white border rounded w-5/6 cursor-pointer flex items-center justify-center m-auto disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={!canClick}
                            onClick={() => {
                                handleClickSubmit();
                            }}
                ...
                {isShowEmoji && <EmojiPicker onEmojiClick={onClickEmoji} />}
            </div>
        </div>
    );
};
