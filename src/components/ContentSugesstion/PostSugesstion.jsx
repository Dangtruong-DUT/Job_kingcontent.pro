...
const PostSugesstion = (props) => {
    ...
    const { page = 1, isLoadingTrends = false } = useSelector((state) => state.trendings);

    const {
        totalPages: totalPagesSpecial,
        currentPage: currentPageSpecial,
        selectedIndexs: selectedIndexsSpecial,
    ...
    const handleSelectContent = (content) => {
        const { medias = [], post_text = "", media_type = "image", videos = [], media_url = "" } = content;
        confirmAlert({
            title: "Xác nhận",
            ...
            buttons: [
                ...
                {
                    label: media_type === "video" ? "Chọn cả nội dung và video" : "Chọn cả nội dung và ảnh",
                    color: "blue",
                    onClick: async () => {
                        dispatch(toggleEditorText(post_text, true));
                        dispatch(
            ...
            {multiSelect == false ? (
                ...
                <div className="flex items-center justify-end space-x-5 pr-8">
                    <span className="font-bold text-lg text-gray-700">
                        {selectedIndexsSpecial.length == 0
                            ? "Chọn tất cả "
                            : "Đang chọn : " + selectedIndexsSpecial.length}
                    </span>
                    <input
                        defaultChecked={false}
                        onChange={(e) => handleMultiSelectContent(-1, e.target.checked)}
                        type="checkbox"
                        className="w-6 h-6 focus:ring-blue-500 rounded"
                    ></input>
                </div>
            )}
          ...
          <LoadingApp />
                    ...
                    <>
                        {contents.map((_elt, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="w-4/6" onClick={() => handleActionShowPopup(_elt)}>
                                        <h3 className="font-bold">{post_header}</h3>
                                        <ContentStyled className="max-w-full overflow-hidden">
                                            <p className="truncate" dangerouslySetInnerHTML={{ __html: post_text }}></p>
                                        </ContentStyled>
                                    ...
                                    <div className="w-1/6 flex items-center justify-center">
                                        {multiSelect == false ? (
                                            ...
                                            <input
                                                checked={selectedIndexsSpecial.includes(index)}
                                                onChange={(e) => handleMultiSelectContent(index, e.target.checked)}
                                                type="checkbox"
                                                className="w-6 h-6 focus:ring-blue-500 rounded"
                                            ></input>
                                        )}
                        ...
                        <div className="flex justify-center w-full mt-2">
                            <button
                                className={`bg-blue-500 text-white px-4 py-2 rounded-md w-1/2 m-3 ${
                                    totalPagesSpecial === currentPageSpecial && isSpecial ? "hidden" : ""
                                }`}
