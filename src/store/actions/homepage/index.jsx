...
import { createContentToHomepage, toggleEditorText, updateProps } from "@/store/actions/createContent";
import { UrlImageToBase64, convertUrlsToBase64 } from "@/pages/createPost/utility";
...
export const getTagsByKeyword =
    ...
    async (dispatch) => {
        try {
            if (keyword) {
                const response = await client.get(`/contents/search?keyword=${keyword}`);
                if (response) {
                    const { data } = response?.data;
                    dispatch({
    ...
    async (dispatch) => {
        try {
            ...
            if (response.status === OK) {
                ...
                const newData = data
                    .filter((_elt) => _elt.medias && _elt.medias.length > 0 && _elt.post_text)
                    .map((_content, index) => {
                        const newText =
...
const _API_HOT_TREND = "/hot-trend-contents?&_limit=12&_start=0&_where[content_contains]=";
...
export const actionGetMyTopic = (cateId) => async (dispatch) => {
    try {
        const response = await client.get(`/categories/${cateId}/contents?_limit=12&page=1`);
        if (response.status === OK) {
            const { data } = response.data.data;
            const newData = data
                ? data.map((_elt) => {
...
export const actionPushContentToCreateContentScreen = (
    ...
    return async (dispatch) => {
        if (!isCreatedContent) {
            toast.info("Hệ thống đang chuyển dữ liệu, vui lòng chờ trong giây lát !", {
                autoClose: false,
            });
            ...
            if (type === "video") {
                ...
                toast.dismiss();
                toast.warning("Đã chuyển dữ liệu xong, vui lòng chọn kế hoạch để tiếp tục soạn thảo !");
            } else {
