...
export const saveChosenCategories = (data) => async (dispatch) => {
  try {
    ...
    toast.success('Theo dÃµi chá»§ Ä‘á» thÃ nh cÃ´ng !');
  } catch (err) {
    console.log('error:', err);
  }
};
...
export const deleteChosenCategories = (id) => async (dispatch) => {
  try {
    ...
    if (response.status === OK) {
      ...
      toast.success('XÃ³a chá»§ Ä‘á» thÃ nh cÃ´ng !');
    }
  } catch (err) {
    console.log('error:', err);
  }
...
export const actionGetMyTopic = (cateId) => async (dispatch) => {
  try {
    ...
    if (response.status === OK) {
      ...
      toast.success('Láº¥y dá»¯ liá»‡u thÃ nh cÃ´ng !');
    } else {
      dispatch({
        type: ACTION_GET_MY_TOPIC_HOMEPAGE,
        payload: [],
...
export const actionPushContentToCreateContentScreen = (
  ...
  return async (dispatch) => {
    if (!isCreatedContent) {
      toast.info(
        'Há»‡ thá»‘ng Ä‘ang chuyá»ƒn dá»¯ liá»‡u, vui lÃ²ng chá» trong giÃ¢y lÃ¡t !',
        {
          autoClose: false,
        }
      );
      ...
      if (type === 'video') {
        ...
        toast.dismiss();
        toast.warning(
          'ÄÃ£ chuyá»ƒn dá»¯ liá»‡u xong, vui lÃ²ng chá»n káº¿ hoáº¡ch Ä‘á»ƒ tiáº¿p tá»¥c soáº¡n tháº£o !'
        );
      } else {
