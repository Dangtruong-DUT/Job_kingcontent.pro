...
import { setIsActivePreset, setSelectFacebookPreset } from "@/store/actions/editor/createContentActions";
...
export const ACTION_UPDATE_KEY_HAS_IN_CONTENT = "ACTION_UPDATE_KEY_HAS_IN_CONTENT";
export const ACTION_SPECIAL_TRACK = "ACTION_SPECIAL_TRACK";
export const ACTION_TOGGLE_POPUP_SELECT_CATE = "ACTION_TOGGLE_POPUP_SELECT_CATE";
export const ACTION_UPDATE_AVAILABLE_CONTENT = "ACTION_UPDATE_AVAILABLE_CONTENT";
    ...
    return async (dispatch, getState) => {
        ...
        if (resKey.status === OK) {
            const arrReq = resKey.data[0].keywords.map((_elt) =>
                Client.get(
                    `${categorySelect.parent.old_id}-${API_FIND_BY_KEYWORD}?content_contains=${_elt}&category=${
                        categorySelect._id
                    }&_limit=${
                        resKey.data[0].keywords.length > 4
                            ? _LIMIT_CURRENT_SMALL
...
export const actionGetPageByKeyword = (api, query, keyword, startDocument = 0) => {
    return async (dispatch, getState) => {
        const { categorySelect } = getState()["createPost"];
        dispatch({
            type: RESET_TRENDING,
...
export const actionGetPostByKeywordTrending = (key) => {
    return async (dispatch) => {
        dispatch(actionLoadingApp(true));
        const res = await Client.get(`${API_FIND_BY_KEYWORD}?content_contains=${key}&_limit=20&_start=0`);
        if (res.status === OK) {
            dispatch({
                type: ACTION_GET_TITLE_CHILD,
                payload: res.data || [],
...
export const actionUpdateEditorState = (text) => {
    return (dispatch, getState) => {
        ...
        keyworksCheck.forEach((_elt) => {
            if (textContent.includes(_elt.original) && !keyworksCheck.includes(_elt.original)) {
    ...
    return (dispatch, getState) => {
        ...
        dispatch(
            actionReplaceContent(
                EditorState.push(editorState, ContentState.createFromBlockArray(convertFromHTML(text || "")))
            )
        );
        const { keyworksCheck, textContent } = getState()["createPost"];
        let keyHas = [];
...
export const createPost = (hasSchedule = false, isReels = false) => {
    return async (dispatch, getState) => {
        ...
        if (!isObjEmpty(editItem)) {
            ...
            updatedData.preset_id = isActivePreset ? selectedFacebookPreset || "" : "";
            ...
            newContentData.preset_id = isActivePreset ? selectedFacebookPreset || "" : "";
...
export const ACTION_UPDATE_SYNC_REQUEST_LIST = "ACTION_UPDATE_SYNC_REQUEST_LIST";
...
export const getBankQuestion = () => {
    return async (dispatch) => {
        const { data: questionData, status } = await CreateContent.getBankQuestion();
        if (status === OK) {
            dispatch({
...
export const getQuestionPromt = (id) => {
    return async (dispatch) => {
        const { data: questionData, status } = await CreateContent.getQuestionInCate(id);
        if (status === OK) {
            dispatch({
                type: ACTION_GET_BANK_QUESTION,
                payload: questionData.data,
    ...
    return async (dispatch) => {
        ...
        if (status === OK) {
            const _newTagArray = historyData.data
                .filter((_elt, index) => historyData.data.findIndex((item) => item.tag === _elt.tag) === index)
                .map((_item) => {
    ...
    return async (dispatch) => {
        ...
        if (status === OK) {
            ...
            const _newTagArray = historyData
                .filter((_elt, index) => historyData.findIndex((item) => item.tag === _elt.tag) === index)
                .map((_item) => {
                    return _item.tag ? _item.tag : null;
                });
            dispatch({
...
export const ACTION_UPDATE_PROPS_CREATE_CONTENT = "ACTION_UPDATE_PROPS_CREATE_CONTENT";
...
export const actionGetPlans = (setOriginalPlans) => {
    return async (dispatch) => {
        ...
        if (res.status === OK) {
            const arr = uniqueObjInArray(res.data.data, "hashtag").map((_elt) => _elt.hashtag);
            const sugg_hash = [
                "Content bán hàng",
...
export const deletePostTextWithAdminSuggestion = (data) => {
    ...
    if (data.labels && Array.isArray(data.labels)) {
        ...
        data.labels.forEach((label) => {
            label.contents = label.contents.filter((content) => content.post_text !== "ADMIN_SUGGESTION");
...
export const resetCreateContent = () => {
    return (dispatch, getState) => {
        ...
        dispatch(actionUpdateEditorState(EditorState.push(editorState, ContentState.createFromText(""))));
    };
};
...
export const actionGetSuggChatGPT = (label_id, product_name) => {
    return async (dispatch, getState) => {
        const _requestPedding = getState()["createPost"].syncRequestPedding;
        const { [KEY_PLAN_DATA]: contentInPlan, [KEY_PLAN_SELECT]: planSelect } = getState()[REDUX_NAME_CREATE_POST];
        _requestPedding.map(async (question, index) => {
            if (question.status === "REJECT") {
                dispatch(actionLoadingApp(true));
                const id = Math.floor(Math.random() * Math.floor(Math.random() * Date.now()));
                ...
                await Client.post(`${API_QUESTION_CHAT_GPT}`, {
                    ...
                    .then(async (response) => {
                        const requestPedding = getState()["createPost"].syncRequestPedding;
                        const requests = requestPedding.filter((_item) => _item.id !== id);
                        const requestsFind = requestPedding.filter((_item) => _item.id === id);
                        await dispatch(actionUpdateSyncRequestPedding(requests));
                        const answerRevert = response?.data?.data || "";
                        /*const anwerRevert = (response.data.data + '').replace(
              /([^>\r\n]?)(\r\n|\n\r|\r|\n)/g,
              '$1' + '<br />' + '$2'
                        ...
                        if (planSelect.id === requestsFind[0]?.planId) {
                            ...
                            const labelsFind = newData.labels.findIndex((_elt) => _elt.id === label_id);
...
export const actionOptimizeThreads = (text, existItem = null) => {
    return async (dispatch, getState) => {
        ...
        if (!existItem) {
            const currentThreadsChatGptData = getState()["createPost"].threadsChatGptData;
            // push to redux threadsChatGptData with status = 'PENDDING'
            const newItem = {
                id: Math.floor(Math.random() * Math.floor(Math.random() * Date.now())),
        ...
        // call api
        const res = await Client.post(`${API_QUESTION_CHAT_GPT}`, {
            question: currentItem.title,
        })
            .then((response) => {
                const newThreadsChatGptData = getState()["createPost"].threadsChatGptData;
                ...
                dispatch(updateProps([{ prop: "threadsChatGptData", value: newThreadsChatGptDataAfter }]));
            })
            .catch((error) => {
                const newThreadsChatGptData = getState()["createPost"].threadsChatGptData;
                const newThreadsChatGptDataAfter = newThreadsChatGptData.filter((item) => item.id !== newItem.id);
                dispatch(updateProps([{ prop: "threadsChatGptData", value: newThreadsChatGptDataAfter }]));
                toast.error("Có lỗi xảy ra . Vui lòng thử lại !");
            });
    };
